...
import { hasBetaAccess, isBetaUser, isDeveloperUser } from '../services/user-type-service.js';
...
router.get('/beta-access', authenticateUser, async (req, res) => {
  try {
    ...
    const hasAccess = hasBetaAccess(userEmail);
    const isBeta = isBetaUser(userEmail);
    const isDeveloper = isDeveloperUser(userEmail);
    ...
    res.json({
      ...
      data: {
        ...
        userType: isDeveloper ? 'developer' : isBeta ? 'beta' : 'standard'
      }
    });
  } catch (error) {
