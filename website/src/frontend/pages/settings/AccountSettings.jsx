...
export default function AccountSettings() {
  ...
  useEffect(() => {
    ...
    if (
      tab &&
      ["general", "security", "notifications", "membership", "email"].includes(
        tab,
      )
    ) {
      setActiveTab(tab);
  ...
  const handleSave = async (tab) => {
    try {
      ...
      switch (tab) {
        case "general":
          // Update user metadata for general settings
          const {
            data: { user },
          } = await useSupabase().auth.updateUser({
            data: {
              full_name: `${generalSettings.firstName} ${generalSettings.lastName}`,
        ...
        case "security":
          ...
          result = await settingsService.updatePreference(
            "security",
            securitySettings,
          );
          break;
        ...
        case "notifications":
          // Save notification settings to user settings
          result = await settingsService.updateUserSettings({
            ...
            call_reminders: notificationSettings.callReminders,
            meeting_reminders: notificationSettings.meetingReminders,
          });
          break;

        case "appearance":
          // Save appearance settings to user settings
          result = await settingsService.updateUserSettings({
        ...
        case "membership":
          // Membership settings are read-only for now
          result = { success: true, data: membershipSettings };
          break;

        case "email":
          // Save email preferences to user preferences
          result = await settingsService.updatePreference("email_preferences", {
            send_as_name:
              generalSettings.firstName + " " + generalSettings.lastName,
            weekly_digest: true,
          });
          break;

        default:
          result = { success: true, data: generalSettings };
      }
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1">
              <div className="bg-white rounded-lg border border-crm-border p-6">
                ...
                {activeTab === "general" && (
                  <div>
                    <h2 className="text-xl font-semibold text-crm-text-primary mb-6">
                      General Settings
                    </h2>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {membershipSettings.permissions.map(
                            (permission, index) => (
                              <div
                                key={index}
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <Input
                          type="text"
                          value={
                            generalSettings.firstName +
                            " " +
                            generalSettings.lastName
                          }
