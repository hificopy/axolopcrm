...
class AutomationEngine {
    ...
    try {
      ...
      const { data: pendingExecutions, error } = await supabase
        ...
        .select(
          `
          *,
          workflow:automation_workflows(*, automation_workflow_steps(*))
        `,
        )
        .eq("status", "PENDING")
        .limit(10);
      ...
      for (const execution of pendingExecutions) {
        try {
          ...
          console.error(
            `Error executing workflow ${execution.workflow_id}:`,
            error,
          );
        }
      }
  ...
  async executeEmailStep(step, execution) {
    ...
    try {
      ...
      if (
        execution.trigger_entity_type === "LEAD" &&
        ...
        execution.trigger_entity_type === "CONTACT" &&
        execution.trigger_entity_id
      ) {
  ...
  async executeWaitForEventStep(step, execution) {
    ...
    try {
      ...
      console.log(
        `Waiting for event: ${eventType} with timeout: ${timeoutMinutes} minutes`,
      );
  ...
  async executeBranchConditionStep(step, execution) {
    ...
    try {
      ...
      if (
        ...
        execution.trigger_entity_type === "DEAL" &&
        execution.trigger_entity_id
      ) {
        const { data, error } = await supabase
          .from("deals")
