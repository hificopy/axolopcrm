...
import { getStripeSecretKey, logStripeMode } from "../utils/stripe-env.js";
...
export async function handleCheckoutCompleted(session) {
  try {
    ...
    // Handle user-based checkout (from SelectPlan flow)
    if (userId && !agencyId) {
      logger.info(`Handling user-based trial checkout for user ${userId}`);

      // Update user metadata with trial info
      const { data: user } = await supabase.auth.admin.getUserById(userId);

      if (user) {
        const updatedMetadata = {
          ...user.user.user_metadata,
          subscription_status: stripeSubscription.status, // "trialing" or "active"
          trial_claimed: true,
        ...
        logger.info(`Updated user ${userId} metadata with trial info`);
      }

      return { success: true };
    }

    // Handle agency-based checkout (original flow)
    if (agencyId) {
    ...
    logger.warn("No agency_id or user_id in checkout session metadata");
    return { success: false, error: "Missing agency_id or user_id" };
  } catch (error) {
...
export async function handleInvoicePaymentFailed(invoice) {
  try {
    ...
    // Check if this is a user-based subscription (from trial checkout)
    // Try to find user by stripe_customer_id in user_metadata
    const { data: users } = await supabase.auth.admin.listUsers();
    const userWithCustomer = users?.users?.find(
      (u) => u.user_metadata?.stripe_customer_id === customerId,
    );

    if (userWithCustomer) {
      // User-based subscription - update user metadata
      logger.info(`Payment failed for user ${userWithCustomer.id}`);
      ...
      await supabase.auth.admin.updateUserById(userWithCustomer.id, {
        user_metadata: updatedMetadata,
      });

      logger.warn(`User ${userWithCustomer.id} marked as past_due with 7-day grace period`);
      return { success: true };
    }

    // Agency-based subscription (original flow)
    const { data: subscription } = await supabase
      .from("subscriptions")
      .select("agency_id")
