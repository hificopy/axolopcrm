...
class EmailService {
  ...
  async sendCampaignEmail(campaignId, recipient, leadId = null, contactId = null) {
    try {
      ...
      const { data: campaign, error: campaignError } = await supabase
        .from('email_campaigns')
                .select(`
                  *,
                  template:email_templates!inner(*)
                `)
        .eq('id', campaignId)
        .single();
    ...
    try {
      const { data: pendingEmails, error: fetchError } = await supabase
        .from('campaign_emails')
                .select(`
                  *,
                  campaign:email_campaigns!inner(*, template:email_templates!inner(*))
                `)
        .eq('status', 'PENDING')
      ...
      for (const email of pendingEmails) {
        try {
          ...
          let recipient = { email: email.recipient_email, name: email.recipient_name };
          ...
          const sendResult = await this.sendEmail({
            ...
            subject: this.replacePlaceholders(email.campaign.subject, recipient),
          ...
          if (failUpdateError) {
            console.error('Error updating failed email status:', failUpdateError);
          }
