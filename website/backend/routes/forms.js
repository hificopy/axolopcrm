  ...
  try {
    ...
    let query = supabaseServer
      ...
      .select(`
        ...
        workflow_nodes, workflow_edges, endings
      `)
...
router.put('/:id', authenticateUser, async (req, res) => {
  try {
    ...
    if (req.body.workflow_nodes !== undefined) updates.workflow_nodes = req.body.workflow_nodes;
    if (req.body.workflow_edges !== undefined) updates.workflow_edges = req.body.workflow_edges;
    if (req.body.endings !== undefined) updates.endings = req.body.endings;
 ...
 * Publish a form (creates a new version)
 */
router.post('/:id/publish', authenticateUser, async (req, res) => {
  try {
    ...
    if (customSlug) {
      ...
      // Check if slug is unique for this user (excluding current form)
      const { count, error: slugError } = await supabaseServer
        .from('forms')
        .select('id', { count: 'exact', head: true })
        .eq('user_id', userId)
      ...
      slug = await generateUniqueSlug(supabaseServer, form.title, userId, id);
    }

    // Get user's agency alias
    ...
    // Increment version
    const newVersion = (form.published_version || 0) + 1;
    ...
    const publishHistoryEntry = {
      ...
      snapshot: {
        ...
        questions: form.questions,
        settings: form.settings,
        workflow_nodes: form.workflow_nodes,
        workflow_edges: form.workflow_edges,
        endings: form.endings
      }
    };

    // Update existing publish_history or create new array
    const updatedHistory = [...(form.publish_history || []), publishHistoryEntry];

    // Update form with publish information
    const { data: publishedForm, error: updateError } = await supabaseServer
      .from('forms')
      .update({
        published_at: new Date().toISOString(),
        published_version: newVersion,
        published_slug: slug,
        publish_history: updatedHistory,
        is_published: true,
        is_active: true,
        updated_at: new Date().toISOString()
      })
    ...
    res.json({
      ...
      slug: slug,
      publicUrl: publicUrl,
      message: `Form published successfully as version ${newVersion}`
    });
  } catch (error) {
 ...
 * Unpublish a form (makes it inaccessible to public)
 */
router.post('/:id/unpublish', authenticateUser, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
 ...
 */
router.get('/:id/publish-history', authenticateUser, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
    ...
    const { data: form, error } = await supabaseServer
      .from('forms')
      .select('id, title, published_version, publish_history')
      .eq('id', id)
      .eq('user_id', userId)
      .is('deleted_at', null)
      .single();
    ...
    res.json({
      ...
      currentVersion: form.published_version || 0,
      history: form.publish_history || []
    });
  } catch (error) {
 ...
 * Get a published form by agency alias and slug (public endpoint - no auth)
 */
router.get('/public/:agencyAlias/:formSlug', async (req, res) => {
  try {
    const { agencyAlias, formSlug } = req.params;

    // Find user by agency alias
    const { data: user, error: userError } = await supabaseServer
      .from('users')
      .select('id')
      .eq('agency_alias', agencyAlias)
      .single();
    ...
    const { data: form, error: formError } = await supabaseServer
      .from('forms')
      .select('id, title, description, questions, settings, workflow_nodes, workflow_edges, endings, published_at, published_version')
      .eq('user_id', user.id)
      .eq('published_slug', formSlug)
 ...
 * Update form slug (only for published forms)
 */
router.put('/:id/slug', authenticateUser, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
    const { slug } = req.body;
