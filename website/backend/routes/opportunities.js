...
router.get("/", protect, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch opportunities", error: error.message });
  }
});
...
router.get("/:id", protect, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch opportunity", error: error.message });
  }
});
...
router.delete("/:id", protect, requireEditPermissions, async (req, res) => {
  try {
    ...
    await opportunityService.deleteOpportunity(userId, opportunityId);
    ...
    if (
      ...
      return res.status(409).json({
        message: "Cannot delete opportunity",
        error: error.message,
