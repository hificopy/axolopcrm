...
import { Settings, Download, Save, Sparkles, LayoutGrid, DollarSign, Users, TrendingUp, Percent, X, FileText, File, Lock, Unlock, ArrowLeftRight, ArrowUpDown } from 'lucide-react';
...
import dashboardDataService from "@/services/dashboard-wrapper";
...
// Helper function to detect and fix overlapping widgets
const removeOverlaps = (layout, cols = 12) => {
  ...
  // Track occupied cells: { "x,y": true }
  const occupied = new Set();

  // Function to check if a widget overlaps with occupied cells
  const hasOverlap = (widget) => {
  ...
  sorted.forEach(widget => {
    if (hasOverlap(widget)) {
      // Find new position
      const newPos = findNextPosition(widget, widget.y);
      widget.x = newPos.x;
      widget.y = newPos.y;
    }
    markOccupied(widget);
...
export default function Dashboard() {
  ...
  // Track if default preset has been modified
  const [defaultModified, setDefaultModified] = useState(false);
  ...
  useEffect(() => {
    if (currentPreset.startsWith('custom-')) {
      ...
      if (user) {
        ...
        dashboardPresetService.getPreset(customPresetId)
          .then(preset => {
            if (preset) {
              // Remove any overlaps before setting layout
              setLayout(removeOverlaps(preset.layout, 12));
            } else {
              // Fallback to default if custom preset not found
              const defaultPreset = getPreset('default');
              setLayout(removeOverlaps(defaultPreset.widgets, 12));
            }
          })
          .catch(error => {
      ...
      if (preset && preset.widgets && preset.widgets.length > 0) {
        setLayout(removeOverlaps(preset.widgets, 12));
      } else {
  ...
  const handleExitEditMode = () => {
    ...
    const hasUnsavedChanges = savedLayout.length > 0 &&
  ...
  const handleLayoutChange = useCallback((newLayout, allLayouts) => {
    // Only process layout changes in edit mode
    if (!isEditMode || !allLayouts || !allLayouts.lg) {
      return;
    }

    // Update the layout with the new positions from the large breakpoint
    setLayout(prevLayout => {
      ...
      const hasChanges = updatedLayout.some((widget, idx) => {
      ...
      if (hasChanges) {
        // Mark as custom preset if not already
        if (!currentPreset.startsWith('custom-')) {
          setTimeout(() => {
  ...
  }, [isEditMode, currentPreset, customCount]);
  ...
  const handleSaveFromModal = async (name, description) => {
    ...
    try {
      ...
      toast({
        title: "Save Failed",
        description: error.message || "Failed to save dashboard preset. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .data-section h2 {
      ...
      border-bottom: 2px solid #791C14;
    }
    .data-grid {
      display: grid;
...
<body>
  ...
  const handleExportPDF = async () => {
    ...
    try {
      toast({
        ...
        description: "Please wait while we create your beautiful dashboard report.",
      });
  ...
  }, [dashboardData, timeRange]); // Add dependencies that affect widget rendering
  ...
  if (loading || userLoading) {
    return (
      <div className="h-full min-h-screen flex items-center justify-center bg-white pt-[150px]">
        <div className="text-center">
          <div className="relative">
            <div className="animate-spin rounded-full h-16 w-16 border-4 border-gray-200 border-t-[#791C14] mx-auto mb-6"></div>
  ...
  return (
    <div className="h-full flex flex-col crm-page-wrapper bg-white">
      ...
      <div className="relative bg-white border-b border-gray-200 px-4 sm:px-6 py-6">
        <div className="relative flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-[#791C14]/10 to-[#a03a2e]/10 border border-[#791C14]/30">
                  <div className="w-2 h-2 rounded-full bg-[#791C14] animate-pulse" />
                  <span className="text-xs font-bold text-[#791C14] uppercase tracking-wider">{getPeriodLabel(timeRange)}</span>
                </div>
              </div>
          ...
          <div className="crm-button-group overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:pb-0">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="default" className="gap-2 hover:bg-[#791C14]/5 transition-colors">
              ...
              <DropdownMenuContent align="end" className="w-72 shadow-lg border-gray-200">
                ...
                {presetList.map((preset) => (
                  <DropdownMenuItem
                    ...
                    className={`flex flex-col items-start px-3 py-3 cursor-pointer transition-all hover:bg-[#791C14]/5 ${
                      currentPreset === preset.id ? 'bg-[#791C14]/10 border-l-2 border-[#791C14]' : ''
                    }`}
                  >
                    <div className="flex items-center gap-2 w-full mb-1">
                      <LayoutGrid className="h-3.5 w-3.5 text-[#791C14]" />
                ...
                {customPresets.length > 0 && (
                  <>
                    ...
                    <DropdownMenuLabel className="text-sm font-semibold text-gray-900 px-3 py-2 flex items-center gap-2">
                      <Sparkles className="h-3.5 w-3.5 text-[#791C14]" />
                      My Custom Presets
                    </DropdownMenuLabel>
                    {customPresets.map((preset) => (
                      <DropdownMenuItem
                        ...
                        className={`flex flex-col items-start px-3 py-3 cursor-pointer transition-all hover:bg-[#791C14]/5 ${
                          currentPreset === `custom-${preset.id}` ? 'bg-[#791C14]/10 border-l-2 border-[#791C14]' : ''
                        }`}
                      >
                        <div className="flex items-center gap-2 w-full mb-1">
                          <Users className="h-3.5 w-3.5 text-[#791C14]" />
                          <span className="font-semibold text-sm">{preset.name}</span>
                        </div>
                        {preset.description && (
                          <span className="text-xs text-gray-600 ml-5">{preset.description}</span>
      ...
      {/* Edit Mode Banner */}
      {isEditMode && (
        <div className="bg-[#791C14] text-white px-4 py-3 text-sm flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
              ...
              <span className="hidden sm:inline text-white/90 text-xs">
                • Drag widgets to rearrange • Resize from corners • Click lock icons to freeze size
              </span>
              {savedLayout.length > 0 && JSON.stringify(layout) !== JSON.stringify(savedLayout) && (
        ...
        <div className="p-2 sm:p-4 md:p-6 max-w-full h-full">
          {layout.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-96 text-center bg-white rounded-2xl border-2 border-dashed border-gray-300 shadow-sm">
              <LayoutGrid className="h-20 w-20 text-[#791C14]/30 mb-6" />
            ...
            {layout.map((widget, index) => (
              <div
                ...
                className={`dashboard-widget bg-white rounded-xl transition-all duration-300 border border-gray-100 ${
                  isEditMode
                    ? 'hover:shadow-2xl hover:scale-[1.02] ring-2 ring-[#791C14]/20 hover:ring-[#791C14]/40'
                    : 'shadow-md hover:shadow-xl hover:border-[#791C14]/20'
                }`}
                style={{
              ...
              >
                {isEditMode && (
                  <div className="drag-handle absolute -top-1 left-0 right-0 h-8 bg-[#791C14] rounded-t-xl cursor-move flex items-center justify-between px-3 z-20 shadow-lg hover:bg-[#6b1a12] transition-all duration-300">
                    ...
                    <div className="flex items-center gap-0.5">
                      ...
                      <button
                        ...
                        className={`text-white transition-all duration-200 p-1 rounded text-xs ${
                          lockedDimensions[widget.i]?.width
                            ? 'bg-white/30 hover:bg-white/40 ring-1 ring-white/50'
                            : 'hover:bg-white/20'
                        }`}
                        aria-label={lockedDimensions[widget.i]?.width ? "Width locked - Click to unlock" : "Lock width"}
                        title={lockedDimensions[widget.i]?.width ? "Width locked - Click to unlock" : "Lock width"}
                      >
                        {lockedDimensions[widget.i]?.width ? (
                      ...
                      <button
                        ...
                        aria-label={lockedDimensions[widget.i]?.height ? "Height locked - Click to unlock" : "Lock height"}
                        title={lockedDimensions[widget.i]?.height ? "Height locked - Click to unlock" : "Lock height"}
                      >
                        {lockedDimensions[widget.i]?.height ? (
                          <Lock className="h-3.5 w-3.5" />
                        ) : (
