...
const SignIn = () => {
  ...
  // Helper function to determine redirect destination based on user status
  const getUserDestination = (signedInUser) => {
    if (!signedInUser) return "/signin";

    const userEmail = signedInUser.email?.toLowerCase() || "";
    const isGodUser = GOD_EMAILS.includes(userEmail);

    // God users always go to app/home
    if (isGodUser) {
      console.log("[SignIn] God user detected - redirecting to /app/home");
      return "/app/home";
    }

    const subscriptionStatus =
      signedInUser.user_metadata?.subscription_status || "none";
    const gracePeriodEnds = signedInUser.user_metadata?.grace_period_ends_at;

    // Past due users locked to billing
    if (subscriptionStatus === "past_due") {
      // Check if grace period expired
      if (gracePeriodEnds && new Date() > new Date(gracePeriodEnds)) {
        console.log("[SignIn] Grace period expired - redirecting to /select-plan");
        return "/select-plan";
      }
      console.log("[SignIn] Past due user - redirecting to /app/settings/billing");
      return "/app/settings/billing?locked=true";
    }

    // Active trial or paid users go to app
    if (
      subscriptionStatus === "trialing" ||
      subscriptionStatus === "active"
    ) {
      console.log(`[SignIn] ${subscriptionStatus} user - redirecting to /app/home`);
      return "/app/home";
    }

    // Free users must select plan
    console.log("[SignIn] Free user - redirecting to /select-plan");
    return "/select-plan";
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Smart redirect based on user subscription status
      const destination = getUserDestination(result.user);
