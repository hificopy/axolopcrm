  ...
  return (
    <button
      ...
      className={`w-full text-left p-3 transition-all border-b border-gray-100 last:border-b-0 group ${
        isHighlighted ? "bg-blue-50 border-gray-200" : "hover:bg-gray-50"
      }`}
    >
...
export const UniversalSearch = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    console.log('[SEARCH DEBUG] UniversalSearch component mounted, isOpen:', isOpen);
  }, []);
  ...
  // Get agency context at component level (hooks must be called at top level)
  ...
  const handleSearch = useCallback(
    async (searchQuery) => {
      ...
      try {
        console.log('[SEARCH DEBUG] Searching for:', trimmedQuery);
        const response = await searchApi.search(trimmedQuery, {
          limit: 8,
        });
        console.log('[SEARCH DEBUG] Response received:', response);

        if (response.data?.success) {
          const rawResults = response.data.data || {};

          // Get user tier for filtering (getCurrentTier is from useAgency hook at component level)
          const userTier = getCurrentTier ? getCurrentTier() : 'free';
        ...
        console.error("[SEARCH DEBUG] Error response:", error.response);
        toast({
          title: "Search Error",
          description:
  ...
  const handleSettingsCommand = useCallback(
    (setting) => {
      const settingsMap = {
        profile: "/app/profile",
        user: "/app/profile",
        agency: "/app/settings/agency",
        team: "/app/settings/agency/team",
        security: "/app/settings/security",
        notifications: "/app/settings/notifications",
        integrations: "/app/settings/integrations",
        billing: "/app/settings/billing",
        plans: "/app/settings/billing",
      };
  ...
  // Debounced search
  useEffect(() => {
    console.log('[SEARCH DEBUG] useEffect triggered, query:', query);
    const timeoutId = setTimeout(() => {
      console.log('[SEARCH DEBUG] Debounce timeout completed, query:', query);
      if (query.trim()) {
        console.log('[SEARCH DEBUG] Calling handleSearch with:', query);
        handleSearch(query);
      } else {
    ...
    return () => {
      console.log('[SEARCH DEBUG] Cleanup timeout');
        ...
        <div className="max-h-96 overflow-y-auto">
            ...
            <div className="p-8 text-center">
              <div className="relative mb-4">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <Search className="h-4 w-4 text-[#3F0D28]" />
                </div>
              </div>
          ...
          {!loading &&
            ...
            showQuickActions && (
              <div className="p-8 text-center">
                <div className="relative mb-4">
                  <Zap className="h-12 w-12 text-[#3F0D28] mx-auto" />
