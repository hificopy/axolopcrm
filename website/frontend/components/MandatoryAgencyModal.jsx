...
export default function MandatoryAgencyModal({ isOpen }) {
  ...
  // Check if this tab can show the modal (only master tab for mandatory modals)
  useEffect(() => {
    if (isOpen) {
      const shouldShow = canShowModal("mandatory");
      setCanShow(shouldShow);

      // Broadcast modal state to other tabs
      broadcastModalState("mandatory", true);

      console.log(
        `[MandatoryAgencyModal] Modal isOpen: ${isOpen}, canShow: ${shouldShow}, isMaster: ${isMasterTab()}`,
      );
    } else {
      setCanShow(false);
      broadcastModalState("mandatory", false);
    }
  }, [isOpen]);

  // Listen for modal state changes from other tabs
  useEffect(() => {
    const handleModalStateChange = (data) => {
      if (data.modalType === "mandatory") {
        console.log(
          `[MandatoryAgencyModal] Modal state changed from another tab:`,
          data,
        );
        // If another tab opened the modal and we're not master, hide ours
        if (data.isOpen && !isMasterTab()) {
          setCanShow(false);
        }
      }
    };

    onTabEvent("modal-state-changed", handleModalStateChange);

    return () => {
      offTabEvent("modal-state-changed", handleModalStateChange);
    };
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (name.length > 100) {
      setError("Agency name must be less than 100 characters");
      setLoading(false);
      return;
    }
    ...
    try {
      console.log(
        "[MandatoryAgencyModal] Starting agency creation with data:",
        {
          name: data.name,
      ...
      // But if it doesn't close in 3 seconds, something went wrong with state update
      // Only reload if this is the master tab to avoid conflicts
      setTimeout(() => {
        if (isOpen && isMasterTab()) {
          console.warn(
            "[MandatoryAgencyModal] Modal still open after 3s - forcing page reload (master tab only)",
          );
          window.location.reload();
        } else if (isOpen && !isMasterTab()) {
          console.log(
            "[MandatoryAgencyModal] Modal still open but not master tab - waiting for master tab to handle",
          );
        }
      }, 3000);
    } catch (err) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div
        className="border border-gray-700 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl shadow-white/10 relative overflow-hidden"
        style={{
          background:
        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-gray-400 bg-gray-800/50 border border-white/10 rounded-lg p-2.5 mt-3">
              ...
              <p>
                All your data (leads, contacts, forms, etc.) is stored within
                your agency. You need an agency to start using Axolop CRM.
              </p>
            </div>
          </div>
          ...
          {/* Error Message */}
          {error && (
            <div className="mb-4 p-3 bg-[#3F0D28]/10 border border-[#3F0D28]/20 rounded-lg">
              <p className="text-[#3F0D28] text-xs">{error}</p>
            </div>
          )}
          ...
          <div className="mt-4 pt-3 border-t border-white/10">
            <p className="text-xs text-gray-400 text-center">
              Your agency will be your workspace where all your data is
