...
export default function Onboarding() {
  ...
  // Kate's special onboarding
  const [showKateOnboarding, setShowKateOnboarding] = useState(false);
    ...
    const checkCurrentUser = async () => {
      try {
        ...
        if (supabaseUrl && supabaseAnonKey) {
          ...
          if (session?.user) {
            // Check if the current user is Kate
            if (session.user.email === 'kate@kateviolet.com') {
              setCurrentUser(session.user);
              // Always show Kate's onboarding for her account on the onboarding page
              // And reset the completion status so it shows every time
              localStorage.removeItem('kateOnboardingCompleted');
              setShowKateOnboarding(true);
            } else {
              setCurrentUser(session.user);
            }
          }
  ...
  useEffect(() => {
    ...
    if (fname) {
      setAffiliateName(fname.charAt(0).toUpperCase() + fname.slice(1).toLowerCase());
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL || 'http://localhost:3002/api'}/forms?title=${
            affiliateRef ? 'CRM Onboarding - Affiliate Flow' : 'CRM Onboarding - Regular Flow'
          }`
        );
  ...
  const calculateRecommendation = (responses) => {
    ...
    if (automation === 'No automation, everything is manual' || automation === 'Basic automation, simple email sequences') {
      score.starter += 5;
      score.professional += 8;
    } else if (automation === 'Intermediate, multistep workflows with conditions') {
      score.professional += 5;
      score.agency += 10;
    } else {
      score.agency += 10;
    ...
    if (businessType === 'GoHighLevel Agency' || businessType === 'Marketing/Advertising Agency') {
      score.agency += 15;
      score.professional += 10;
    }

    // Find highest score
    const maxScore = Math.max(score.starter, score.professional, score.agency, score.enterprise);
  ...
  const getCurrentSectionInfo = (questionIndex) => {
    return sections.find(
      section => questionIndex >= section.startQuestion && questionIndex <= section.endQuestion
    );
  };
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {[...Array(20)].map((_, i) => (
              <motion.div
                key={i}
                className="absolute w-2 h-2 bg-gradient-to-br from-[#791C14]/20 to-[#ff6b4a]/20 rounded-full"
                initial={{
              ...
              <div className="relative">
                ...
                >
                  ...
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="bg-gradient-to-br from-[#791C14] to-[#ff6b4a] p-8 rounded-full"
                  >
  ...
  // Special onboarding for Kate - after completion continue to regular onboarding
  if (showKateOnboarding) {
    return (
      <KateOnboarding
        onComplete={() => {
          // Mark Kate's special onboarding as completed for this session
          // This allows her to access the main app until next login
          localStorage.setItem('kateOnboardingCompleted', 'true');
          // Then continue to regular onboarding flow
          setShowKateOnboarding(false);
          setShowIntro(true); // Show the regular onboarding intro
        }}
      />
    );
  }
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            <div className="relative">
              <motion.div
                ...
                className="inline-block rounded-full"
              >
                <div className="bg-gradient-to-br from-[#791C14] to-[#ff6b4a] p-8 rounded-full">
          ...
          <motion.h1
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.5, duration: 0.8 }}
            className="text-5xl md:text-6xl font-bold bg-gradient-to-r from-[#791C14] to-[#ff6b4a] bg-clip-text text-transparent mb-4"
          >
          ...
          >
            {[...Array(3)].map((_, i) => (
              <motion.div
                key={i}
                className="w-3 h-3 bg-gradient-to-r from-[#791C14] to-[#ff6b4a] rounded-full"
                animate={{
                  scale: [1, 1.5, 1],
                  opacity: [0.5, 1, 0.5],
                }}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="text-center">
            ...
            >
              <Brain className="w-16 h-16 text-[#791C14]" />
            </motion.div>
      ...
      <AnimatePresence mode="wait">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h1 className="text-4xl font-bold mb-2 text-gray-900">Perfect Match Found!</h1>
            <p className="text-gray-600 text-lg">
              Based on your answers, we've tailored the perfect CRM setup for {responses.name}
            </p>
          </motion.div>
          ...
          >
            ...
            {affiliateRef && (
              <div className="absolute top-0 right-0">
                <div className="bg-gradient-to-r from-[#791C14] to-[#ff6b4a] text-white px-6 py-2 rounded-bl-2xl shadow-lg">
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-3xl font-bold mb-2 text-gray-900">{plan.name} Plan</h2>
                <p className="text-gray-600">{plan.description}</p>
                <p className="text-sm text-gray-500 mt-2">
                  {affiliateRef ? '30-day free trial included' : '14-day free trial included'}
                </p>
              </div>
              <div className="text-right">
                <div className="text-4xl font-bold text-[#791C14]">{plan.price}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-lg text-green-700">
                      Save ${recommendedPlan.estimatedSavings.toLocaleString()}/month
                    </div>
                    <div className="text-sm text-gray-600">
                      That's ${(recommendedPlan.estimatedSavings * 12).toLocaleString()}/year by consolidating your tools
                    </div>
                  </div>
                </div>
              </div>
          ...
          {recommendedPlan.setupRecommendations.length > 0 && (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                <Zap className="w-6 h-6 text-[#791C14]" />
              ...
              <ul className="space-y-2">
                {recommendedPlan.setupRecommendations.map((item, index) => (
                  <li key={index} className="flex items-center gap-3">
                    <Target className="w-5 h-5 text-[#791C14]" />
          ...
          >
            <Button
              ...
              className="relative overflow-hidden bg-gradient-to-r from-[#791C14] to-[#ff6b4a] hover:from-[#8b2c24] hover:to-[#ff7b5a] text-white text-xl px-12 py-8 rounded-xl shadow-xl transition-all duration-300 transform hover:scale-105"
            >
