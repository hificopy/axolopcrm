...
class ComprehensiveSearchService {
  ...
  async searchOpportunities(userId, query, limit) {
    try {
      const { data, error } = await supabase
        ...
        .eq("user_id", userId)
        .or(
          `name.ilike.%${query}%,company.ilike.%${query}%,stage.ilike.%${query}%`,
        )
        .limit(limit);
  ...
  async searchConversationHistory(userId, query, limit) {
    try {
      ...
      return data.map((conv) => ({
        ...
        url: `/app/conversations?conversation=${conv.id}`,
  ...
  async searchUserActivity(userId, query, limit = 8) {
    ...
    const { data, error } = await supabase
      ...
      .eq("user_id", userId)
      .or(
        `action.ilike.%${query}%,entity_type.ilike.%${query}%,page_url.ilike.%${query}%`,
      )
      .order("created_at", { ascending: false })
      .limit(limit);
  ...
  async searchSystemStatus(query, limit = 8) {
    const { data, error } = await supabase
      ...
      .select("*")
      .or(
        `service_name.ilike.%${query}%,status.ilike.%${query}%,message.ilike.%${query}%`,
      )
  ...
  async searchAll(userId, query, options = {}) {
    ...
    if (!userId) {
      ...
      return {
        ...
        totalCount:
          navigationPages.length + helpArticles.length + quickActions.length,
        categories: {
          navigation: navigationPages.length,
