 ...
 */
export default function CreateBookingDialog({ open, onOpenChange, onSave, initialData = null, isEditing = false }) {
  ...
  // Reset form data when dialog opens with initialData
  useEffect(() => {
    if (open) {
      if (initialData && isEditing) {
        // Merge initial data with defaults for editing
        setFormData({
          ...defaultFormData,
          ...initialData,
          ...
          secondaryQuestions: initialData.secondary_questions || initialData.secondaryQuestions || [],
        });
      } else {
  ...
  const handleSave = async () => {
    try {
      ...
      // For new bookings, require at least one host
      if (!isEditing && formData.hosts.length === 0) {
        toast({
          title: 'Missing Required Field',
      ...
      // Toast message is now handled by the parent component
      onOpenChange(false);
    } catch (error) {
      console.error(`Error ${isEditing ? 'updating' : 'creating'} booking link:`, error);
      toast({
        title: 'Error',
        description: `Failed to ${isEditing ? 'update' : 'create'} booking link. Please try again.`,
        variant: 'destructive',
      });
    }
  };
        ...
        <div className="px-6 py-4 border-b">
          <DialogHeader>
            <DialogTitle className="text-2xl">
              {isEditing ? 'Edit Booking Link' : 'Create Booking Link'}
            </DialogTitle>
            <DialogDescription>
              {isEditing ? 'Update your booking link settings' : 'Set up a new booking link for your team'}
            </DialogDescription>
          </DialogHeader>
        </div>
