...
export default function ConnectTab({ form }) { // Removed setForm as it's no longer needed for integrations
  ...
  if (loading) {
    return <div className="h-full flex items-center justify-center text-crm-text-secondary">Loading integrations...</div>;
  }

  if (error) {
    return <div className="h-full flex items-center justify-center text-red-500">Error loading integrations: ${error.message}</div>;
  }
