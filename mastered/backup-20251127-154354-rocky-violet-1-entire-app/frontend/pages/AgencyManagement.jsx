  ...
  useEffect(() => {
    ...
    if (agencies.length > 0) {
      const totalMembers = agencies.reduce(
        (sum, agency) => sum + (agency.current_users_count || 0),
        0,
      );
      const activeMembers = totalMembers; // All members are considered active
      const totalInvites = agencies.reduce(
        (sum, agency) => sum + (agency.pending_invites || 0),
        0,
      );
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#3F0D28] text-white font-medium rounded-lg hover:bg-[#2a0919] transition-colors"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage your agencies, settings, and team members
          </p>
        </div>
        ...
        <div className="mb-8 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          >
            <div className="flex items-center gap-3">
              <Plus className="h-8 w-8 text-[#3F0D28]" />
              <div>
                <h3 className="font-semibold text-gray-900">
                  Create New Agency
                </h3>
                <p className="text-sm text-gray-600">
                  Start a new agency or workspace
                </p>
              </div>
            </div>
          </button>
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-lg border border-gray-200">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="h-8 w-8 text-[#3F0D28] mx-auto mb-2 bg-blue-100 rounded-full flex items-center justify-center">
                  <span className="text-blue-800 font-semibold text-xs">
                    {Math.round(stats.storageUsed)}%
                  </span>
                </div>
          ...
          {agencies.map((agency) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div
                  className={`px-3 py-1 rounded-full text-xs font-medium ${getSubscriptionColor(agency.subscription_tier)}`}
                >
                  {agency.subscription_tier || "Free"}
                </div>
                <div className="text-right">
                  <div className="text-sm text-gray-500">
                    {agency.current_users_count || 0} / {agency.max_users || 3}{" "}
                    members
                  </div>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-gray-200">
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-[#3F0D28] text-white font-medium rounded hover:bg-[#2a0919] transition-colors"
                >
                  Switch to Agency
                </button>
        ...
        {agencies.length === 0 && (
          <div className="text-center py-12">
            <div className="bg-white rounded-lg border border-gray-200 p-8 max-w-md mx-auto">
              <Building2 className="h-16 w-16 text-[#3F0D28] mx-auto mb-4" />
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No Agencies Yet
              </h2>
              <p className="text-gray-600 mb-6">
                You haven't created any agencies yet. Get started by creating
                your first agency.
              </p>
              <button
                onClick={handleCreateAgency}
