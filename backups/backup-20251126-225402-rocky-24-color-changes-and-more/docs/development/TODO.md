...
**Status:** ✅ RESOLVED - All files now use proper Supabase auth via api module

**Files Fixed:**
- [x] `frontend/components/LeadImportModal.jsx` (4 occurrences) - Now uses `api` from `@/lib/api`
- [x] `frontend/pages/CreateCampaign.jsx` (3 occurrences) - Now uses `api` from `@/lib/api`
- [x] `frontend/components/EnhancedLeadImportModal.jsx` (2 occurrences) - Now uses `api` from `@/lib/api`
- [x] `frontend/pages/Contacts.jsx` (1 occurrence) - Now uses `contactsApi` from `@/lib/api`

**Changes Made:**
- Replaced `localStorage.getItem('supabase.auth.token')` with proper api module
- The `api` module automatically handles authentication via Supabase session
- All API calls now properly authenticated without manual token management
