...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    setLoading(true);
    try {
      // Use new unified API with tiered caching
  ...
  // Handle layout change (drag/resize) with optimized comparison
  const handleLayoutChange = useCallback(
    (newLayout) => {
      if (isEditMode) {
        setLayout((prevLayout) => {
          // Use deep comparison instead of expensive JSON.stringify
          if (!deepEqual(prevLayout, newLayout)) {
    ...
    [isEditMode, currentPreset, customCount, deepEqual],
  );
    ...
    <div className="h-full flex flex-col crm-page-wrapper bg-white">
      ...
      <div className="relative bg-white px-6 sm:px-8 py-6">
        <div className="relative flex flex-col gap-4">
          ...
          <div className="crm-button-group overflow-x-auto pb-2 sm:pb-0">
            {/* Time Range Selector */}
            <DropdownMenu>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-black/[0.06] p-6 shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-shadow">
            ...
            <p className="text-sm text-neutral-400 mt-1">
              ${(dashboardData.sales?.totalRevenue || 0).toLocaleString()} All Time
            </p>
          </div>
          {/* Active Leads */}
          <div className="bg-white rounded-2xl border border-black/[0.06] p-6 shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-shadow">
            <p className="text-sm font-medium text-neutral-500 mb-1">Active Leads</p>
            <p className="text-3xl sm:text-4xl font-bold text-neutral-900 tracking-tight">
          ...
          {/* Conversion Rate */}
          <div className="bg-white rounded-2xl border border-black/[0.06] p-6 shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-shadow">
            <p className="text-sm font-medium text-neutral-500 mb-1">Conversion Rate</p>
            <p className="text-3xl sm:text-4xl font-bold text-[#761B14] tracking-tight">
