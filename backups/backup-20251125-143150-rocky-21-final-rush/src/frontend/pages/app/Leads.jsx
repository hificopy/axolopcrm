...
import { getLeadStatusColor, formatDate, formatCurrency } from "@utils/utils";
import EnhancedLeadImportModal from "@components/layout/EnhancedLeadImportModal";
import CreateLeadModal from "@components/layout/CreateLeadModal";
...
import { LeadTableSkeleton, StatsCardSkeleton } from "@components/ui/skeletons";
...
export default function Leads() {
  ...
  const [isEnhancedImportModalOpen, setIsEnhancedImportModalOpen] =
    useState(false);
  ...
  const applyFilters = (filters) => {
    ...
    if (filters.minValue) {
      filtered = filtered.filter(
        (lead) => (lead.value || 0) >= filters.minValue,
      );
    }
    if (filters.maxValue) {
      filtered = filtered.filter(
        (lead) => (lead.value || 0) <= filters.maxValue,
      );
    }
  ...
  const handleConvertToOpportunity = async () => {
    ...
    try {
      await leadsApi.convertToContact(selectedLead.id);
  ...
  const handleCellEdit = async (leadId, columnKey, value) => {
    try {
      await leadsApi.update(leadId, { [columnKey]: value });
  ...
  // Define groups for status-based grouping
    ...
    <div className="h-full flex flex-col crm-page-wrapper bg-white">
      ...
      >
        ...
        <div className="lg:hidden">
          {/* Mobile leads will be rendered by ResponsiveLeadsLayout */}
        </div>
      </ResponsiveLeadsLayout>
          ...
          <div className="p-6 pt-20">
            ...
            <div className="mt-6 space-y-4">
              <div>
                ...
                <Badge
                  variant={getLeadStatusColor(selectedLead.status)}
                  className="mt-1"
                >
                  {selectedLead.status}
                </Badge>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    {selectedLead.address.street && (
                      <div>{selectedLead.address.street}</div>
                    )}
                    {selectedLead.address.city && (
                      <div>
                        {selectedLead.address.city},{" "}
                        {selectedLead.address.state} {selectedLead.address.zip}
                      </div>
                    )}
                    {selectedLead.address.country && (
                      <div>{selectedLead.address.country}</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {Object.keys(selectedLead.custom_fields || {}).length > 0 && (
                <div>
                  ...
                  {Object.entries(selectedLead.custom_fields).map(
                    ([key, value]) => (
