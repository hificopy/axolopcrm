...
export default function Forms() {
  ...
  const loadForms = useCallback(async () => {
    // Don't attempt to load forms if user isn't authenticated
    if (!user) {
    ...
    try {
      ...
      if (err.message.includes('Authentication token is invalid or expired')) {
        toast({
      ...
      else if (err.message.includes('table') || err.message.includes('schema cache')) {
  ...
  }, [statusFilter, searchTerm, toast, user]);

  useEffect(() => {
    // Wait for auth to be loaded before attempting to load forms
    if (!authLoading) {
      loadForms();
    }
  }, [loadForms, authLoading]);
    ...
    return (
      <div className="h-full min-h-screen flex items-center justify-center pt-[150px] p-6 bg-white">
        <div className="text-center">
          ...
          <p className="text-gray-700 font-medium text-lg">Loading authentication...</p>
  ...
  if (error) {
    ...
    return (
      <div className="h-full min-h-screen flex items-center justify-center pt-[150px] p-6">
        <div className="text-center">
          <h2 className="text-xl font-bold text-crm-text-primary mb-2">
            {isAuthError ? "Authentication Required" : "Error Loading Forms"}
          </h2>
          <p className="text-crm-text-secondary mb-4">
            {isAuthError
              ? "Your session has expired. Please sign in to continue using the form builder."
              : error.message}
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Button
              ...
              className={isAuthError ? "bg-[#7b1c14] hover:bg-[#6b1a12]" : ""}
            >
              Go to Sign In
            </Button>
          </div>
