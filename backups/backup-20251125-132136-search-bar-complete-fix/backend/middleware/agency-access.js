...
import { getUserType, getUserPermissions, canEditInAgency, isGodModeUser } from '../services/user-type-service.js';

/**
 * Middleware to check if user has access to an agency
 * Expects agencyId in req.params or req.body or req.query
 * Attaches userType and permissions to request
 */
export const requireAgencyAccess = async (req, res, next) => {
  try {
    ...
    const permissions = await getUserPermissions(userId, agencyId);

    // Attach agency membership, user type, and permissions to request
    req.agencyMembership = membership;
    req.agencyId = agencyId;
    req.userType = userType;
    req.permissions = permissions;
    req.isAgencyAdmin = membership.role === 'admin';
    req.isSeatedUser = membership.role !== 'admin';
