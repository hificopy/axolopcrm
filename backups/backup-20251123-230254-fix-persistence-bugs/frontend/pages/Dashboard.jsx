...
const generateResponsiveLayouts = (baseLayout, lockedDimensions = {}) => {
  ...
  // Small screens (6 columns) - scale down and stack more
  let smY = 0;
  const sm = baseLayout.map(item => {
    const scaledW = Math.min(Math.ceil(item.w / 2), 6);
    const result = {
      i: item.i,
      x: 0,
      y: smY,
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .data-section h2 {
      ...
      border-bottom: 2px solid #7b1c14;
    }
    .data-grid {
      display: grid;
...
<body>
  ...
  return (
    <div className="h-full flex flex-col crm-page-wrapper bg-white">
      ...
      <div className="relative bg-white border-b border-gray-200 px-4 sm:px-6 py-6">
        <div className="relative flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-[#7b1c14]/10 to-[#a03a2e]/10 border border-[#7b1c14]/30">
                  <div className="w-2 h-2 rounded-full bg-[#7b1c14] animate-pulse" />
                  <span className="text-xs font-bold text-[#7b1c14] uppercase tracking-wider">{getPeriodLabel(timeRange)}</span>
                </div>
              </div>
          ...
          <div className="crm-button-group overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:pb-0">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="default" className="gap-2 hover:bg-[#7b1c14]/5 transition-colors">
              ...
              <DropdownMenuContent align="end" className="w-72 shadow-lg border-gray-200">
                ...
                {presetList.map((preset) => (
                  <DropdownMenuItem
                    ...
                    className={`flex flex-col items-start px-3 py-3 cursor-pointer transition-all hover:bg-[#7b1c14]/5 ${
                      currentPreset === preset.id ? 'bg-[#7b1c14]/10 border-l-2 border-[#7b1c14]' : ''
                    }`}
                  >
                    <div className="flex items-center gap-2 w-full mb-1">
                      <LayoutGrid className="h-3.5 w-3.5 text-[#7b1c14]" />
                ...
                {customPresets.length > 0 && (
                  <>
                    ...
                    <DropdownMenuLabel className="text-sm font-semibold text-gray-900 px-3 py-2 flex items-center gap-2">
                      <Sparkles className="h-3.5 w-3.5 text-[#7b1c14]" />
                      My Custom Presets
                    </DropdownMenuLabel>
                    {customPresets.map((preset) => (
                      <DropdownMenuItem
                        ...
                        className={`flex flex-col items-start px-3 py-3 cursor-pointer transition-all hover:bg-[#7b1c14]/5 ${
                          currentPreset === `custom-${preset.id}` ? 'bg-[#7b1c14]/10 border-l-2 border-[#7b1c14]' : ''
                        }`}
                      >
                        <div className="flex items-center gap-2 w-full mb-1">
                          <Users className="h-3.5 w-3.5 text-[#7b1c14]" />
                          <span className="font-semibold text-sm">{preset.name}</span>
                        </div>
                        {preset.description && (
                          <span className="text-xs text-gray-600 ml-5">{preset.description}</span>
      ...
      {/* Edit Mode Banner */}
      {isEditMode && (
        <div className="bg-[#7b1c14] text-white px-4 py-3 text-sm flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
              ...
              <span className="hidden sm:inline text-white/90 text-xs">
                • Drag widgets to rearrange • Resize from corners • Click X to remove
              </span>
        ...
        <div className="p-4 sm:p-6 max-w-full">
          {layout.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-96 text-center bg-white rounded-2xl border-2 border-dashed border-gray-300 shadow-sm">
              <LayoutGrid className="h-20 w-20 text-[#7b1c14]/30 mb-6" />
            ...
            {layout.map((widget, index) => (
              <div
                ...
                className={`dashboard-widget bg-white rounded-xl transition-all duration-300 border border-gray-100 ${
                  isEditMode
                    ? 'hover:shadow-2xl hover:scale-[1.02] ring-2 ring-[#7b1c14]/20 hover:ring-[#7b1c14]/40'
                    : 'shadow-md hover:shadow-xl hover:border-[#7b1c14]/20'
                }`}
                style={{
              ...
              >
                {isEditMode && (
                  <div className="drag-handle absolute -top-1 left-0 right-0 h-8 bg-[#7b1c14] rounded-t-xl cursor-move flex items-center justify-between px-3 z-20 shadow-lg hover:bg-[#6b1a12] transition-all duration-300">
