...
export default function TodoList() {
    ...
    try {
      ...
      if (columnKey === 'text') {
        const response = await axios.put(
          `${API_BASE_URL}/api/user-preferences/todos/${todoId}`,
          { text: value },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        const response = await axios.post(
          `${API_BASE_URL}/api/user-preferences/todos/${todoId}/toggle`,
          {},
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleDuplicate = async (todo) => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/user-preferences/todos`,
        { text: `${todo.text} (Copy)`, completed: false },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-6 bg-gray-50">
        <MondayTable
          ...
          onAddItemToGroup={handleAddTodoToGroup}
