...
class RAGService {
  ...
  validateConfiguration() {
    ...
    if (!hasChroma) {
      logger.error('❌ ChromaDB is not initialized. RAG service will not work properly.');
    }
    
    if (!hasAI) {
      logger.error('❌ No AI provider (OpenAI or Groq) is configured. RAG service will not work properly.');
    }
  ...
  async addDocument(collectionName, documentId, content, metadata = {}) {
    ...
    try {
      ...
      logger.info(`Document added to RAG knowledge base: ${documentId} in collection: ${collectionName}`);
  ...
  async addDocuments(collectionName, documents) {
    ...
    try {
      ...
      logger.info(`Added ${documents.length} documents to RAG knowledge base: ${collectionName}`);
  ...
  async search(collectionName, query, topK = 5, filter = {}) {
    ...
    try {
      ...
      logger.info(`Found ${formattedResults.length} results for query in collection: ${collectionName}`);
   ...
   */
  async generateResponse(collectionName, query, contextCount = 3, systemPrompt = null) {
    if (!this.initialized) {
      throw new Error('RAG service is not properly initialized');
    }
    ...
    try {
      ...
      const searchResult = await this.search(collectionName, query, contextCount);
      ...
      logger.info(`Generated RAG response for query in collection: ${collectionName}`);
  ...
  async deleteDocument(collectionName, documentId) {
    ...
    try {
      ...
      logger.info(`Document deleted from RAG knowledge base: ${documentId} in collection: ${collectionName}`);
  ...
  async getOrCreateCollection(collectionName) {
    ...
    try {
      const collection = await this.chromaService.getOrCreateCollection(collectionName);
      return collection;
    } catch (error) {
   ...
   */
  async addKnowledgeBaseDocument(collectionName, docType, title, content, source = null, additionalMetadata = {}) {
    if (!this.initialized) {
      throw new Error('RAG service is not properly initialized');
    }
    ...
    try {
      ...
      const result = await this.addDocument(collectionName, documentId, content, metadata);

      logger.info(`Knowledge base document added: ${title} (${docType}) to collection: ${collectionName}`);
