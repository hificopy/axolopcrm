...
router.post('/join', async (req, res) => {
  ...
  console.log('🔑 Authorization Header:', req.headers.authorization ? 'Present (Bearer token)' : 'MISSING');
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(401).json({
        error: 'Unauthorized',
        debug: 'User authentication failed - no valid session token'
      });
    }
    ...
    if (error.message?.includes('Affiliate tables are not set up')) {
      ...
      return res.status(500).json({
        ...
        message: 'Please run the affiliate database migration first. See TO-DOS.md for instructions.',
        hint: 'Run `node scripts/run-affiliate-migration.js` after setting up your Supabase credentials',
        debug: error.message
      });
    }

    console.error('🔥 Generic Error - Full Details:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
    console.error('\n=== END DEBUG ===\n');
...
router.put('/payment-details', async (req, res) => {
  try {
    ...
    const result = await affiliateService.updatePaymentDetails(user.id, req.body);
...
router.get('/report', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'Failed to fetch referral report',
      message: error.message,
    });
  }
});
...
router.get('/check-code/:code', async (req, res) => {
  try {
    ...
    if (affiliate.status !== 'active') {
      return res.json({ valid: false, message: 'Affiliate account is not active' });
    }
