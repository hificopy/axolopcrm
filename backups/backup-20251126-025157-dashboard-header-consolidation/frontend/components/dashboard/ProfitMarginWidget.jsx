...
export default function ProfitMarginWidget({ data }) {
  ...
  // Branded red color palette for pie chart
  const chartData = useMemo(() => [
    { name: 'Net Profit', value: netProfit > 0 ? netProfit : 0, color: '#761B14' },
    { name: 'Expenses', value: expenses > 0 ? expenses : 0, color: '#B85450' },
  ], [netProfit, expenses]);
      ...
      <Card className="h-full flex flex-col bg-white border border-black/[0.05] shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] transition-all duration-300 relative overflow-hidden">
        ...
        <CardHeader className="relative z-10">
          <div className="flex items-center gap-3">
            <motion.div
              className="w-10 h-10 rounded-lg flex items-center justify-center shadow-lg bg-gradient-to-br from-[#761B14] to-[#9A392D]"
        ...
        <CardContent className="flex-1 flex flex-col justify-between">
          ...
          <div className="space-y-3 mt-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-[#B85450]">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-base font-bold ${
                  isProfit ? 'text-[#761B14]' : 'text-red-500'
                }`}>
                  {formatCurrency(netProfit)}
                </span>
                {isProfit ? (
                  <ArrowUpRight className="h-4 w-4 text-[#761B14]" />
                ) : (
            ...
            >
              ...
              <span className={`text-lg font-bold ${
                profitMargin >= 20 ? 'text-[#761B14]' :
                profitMargin >= 10 ? 'text-[#9A392D]' : 'text-[#B85450]'
              }`}>
