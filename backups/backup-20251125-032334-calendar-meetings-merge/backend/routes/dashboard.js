...
router.get('/summary', authenticateUser, async (req, res) => {
  try {
    ...
    // Generate cache key
    const cacheKey = cacheService.key('dashboard', 'summary', userId, timeRange);

    // Use cache-aside pattern: check cache first, fetch if miss
    const data = await cacheService.getOrSet(
      cacheKey,
      async () => {
        logger.info('Dashboard cache miss - fetching from database', { userId, timeRange });

        // Fetch all dashboard data in parallel
        const [overview, sales, marketing, recentLeads, recentActivities] = await Promise.all([
    ...
    logger.debug('Dashboard summary served', { userId, cached: !data.fetchedAt || Date.now() - new Date(data.fetchedAt).getTime() > 1000 });
...
router.post('/invalidate', authenticateUser, async (req, res) => {
  try {
    ...
    await cacheService.delPattern(cacheService.key('dashboard', '*', userId, '*'));
    ...
    logger.error('Error invalidating dashboard cache', { error: error.message });
    res.status(500).json({
      success: false,
