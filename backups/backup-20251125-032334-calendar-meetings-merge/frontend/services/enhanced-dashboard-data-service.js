...
class EnhancedDashboardDataService {
   ...
   * Now tries cached backend endpoint first (10 min Redis cache)
   */
  async getAllMetrics(timeRange = "month") {
    const startTime = Date.now();

    try {
      // Try cached backend endpoint first (much faster!)
      if (this.useBackendCache) {
        const cachedData = await this.originalService.getCachedDashboardSummary(timeRange);
        if (cachedData) {
          const responseTime = Date.now() - startTime;
          console.log(`✅ [METRICS] Dashboard loaded from cache in ${responseTime}ms`);

          // Transform cached backend response to match expected format
          return {
            sales: {
              ...cachedData.sales,
              _metadata: { isReliable: true, status: "cached" },
            },
            marketing: {
              ...cachedData.marketing,
              _metadata: { isReliable: true, status: "cached" },
            },
            profitLoss: {
              revenue: cachedData.sales?.deals?.revenue || 0,
              expenses: 0,
              netProfit: cachedData.sales?.deals?.revenue || 0,
              _metadata: { isReliable: true, status: "cached" },
            },
            forms: {
              total: cachedData.overview?.forms?.submissions || 0,
              _metadata: { isReliable: true, status: "cached" },
            },
            overview: cachedData.overview,
            recentLeads: cachedData.recentLeads,
            recentActivities: cachedData.recentActivities,
            _metadata: {
              responseTime,
              lastValidated: new Date().toISOString(),
              source: "backend_cache",
              overallReliability: { total: 4, reliable: 4, reliabilityPercentage: 100 },
            },
          };
        }
      }

      // Fallback: Execute all metrics methods in parallel (slower, no cache)
      console.log("⚠️ [METRICS] Falling back to direct Supabase queries");
