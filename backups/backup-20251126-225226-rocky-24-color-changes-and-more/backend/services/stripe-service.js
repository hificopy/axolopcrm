...
export async function getOrCreateCustomer(agencyId, email, name) {
  try {
    ...
    logger.info(`Created Stripe customer ${customer.id} for agency ${agencyId}`);
    return { success: true, customer };
  } catch (error) {
 ...
 */
export async function createCheckoutSession(agencyId, tier, billingCycle, userEmail, userName, successUrl, cancelUrl) {
  try {
    // Get or create customer
    const { customer, error: customerError } = await getOrCreateCustomer(agencyId, userEmail, userName);
    ...
    const priceInCents = billingCycle === 'yearly' ? tierConfig.yearly : tierConfig.monthly;
    ...
    logger.info(`Created checkout session ${session.id} for agency ${agencyId}`);
 ...
 */
export async function createTrialSubscription(agencyId, tier = 'sales', billingCycle = 'monthly') {
  try {
...
export async function changeTier(agencyId, newTier, billingCycle = null) {
  try {
    ...
    const priceInCents = newBillingCycle === 'yearly' ? tierConfig.yearly : tierConfig.monthly;
    ...
    logger.info(`Changed tier for agency ${agencyId} from ${subscription.tier} to ${newTier}`);
    return { success: true };
  } catch (error) {
