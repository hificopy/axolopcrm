...
import { DollarSign, TrendingUp, Users, Target, Award, Calendar, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function FullSalesWidget({ data = {}, timeRange = 'month' }) {
  const stats = [
    {
      label: 'Total Revenue',
      value: data.totalRevenue ? `$${(data.totalRevenue / 1000).toFixed(0)}k` : '$0',
      icon: DollarSign,
      color: 'text-[#7b1c14] dark:text-[#d4463c]',
  ...
  const details = [
    { label: 'Avg. Deal Size', value: data.avgDealSize ? `$${(data.avgDealSize / 1000).toFixed(1)}k` : '$0' },
    { label: 'Win Rate', value: data.winRate ? `${data.winRate.toFixed(1)}%` : '0%' },
    { label: 'Sales Cycle', value: data.avgSalesCycle || '0 days' },
    { label: 'Pipeline Value', value: data.pipelineValue ? `$${(data.pipelineValue / 1000).toFixed(0)}k` : '$0' },
  ];
      ...
      <Card className="h-full w-full flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-crm-text-secondary">Complete sales metrics</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900/50">
              ...
              <span className="text-xs font-semibold text-green-700 dark:text-green-300">{getPeriodLabel(timeRange)}</span>
            </div>
          </div>
        ...
        <CardContent className="flex-1 space-y-4">
          ...
          <div className="border-t border-crm-border pt-3 space-y-2">
            <h4 className="text-xs font-semibold text-crm-text-secondary uppercase tracking-wider">Additional Metrics</h4>
