...
export default function MainLayout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden transition-colors duration-300 ease-out">
      ...
      {/* Main Content Area */}
      <div className={`flex-1 flex flex-col min-w-0 transform-gpu transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'ml-0' : 'ml-64'}`}>
        {/* Topbar - should remain unchanged */}
        <Topbar />

        {/* Page Content - dashboard area that changes with theme */}
        <main className={`flex-1 transition-colors duration-300 ease-out overflow-y-auto ${theme === 'dark' ? 'bg-[#0d0f12] text-white' : 'bg-gray-50 text-gray-900'}`}>
          <div className="zoom-container">
            <div className="zoomed-out-content">
