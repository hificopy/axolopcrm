...
if (config.google.enabled) {
  logger.info('✅ Google Auth service available');
} else {
  logger.warn('⚠️ Google Auth service not configured');
}
...
if (config.posthog.enabled) {
  ...
  logger.info('✅ PostHog analytics service initialized');
} else {
  logger.warn('⚠️ PostHog analytics not configured');
}
...
if (config.sentry.enabled) {
  ...
  logger.info('✅ Sentry error tracking service initialized');
} else {
  logger.warn('⚠️ Sentry error tracking not configured');
}

// SendGrid initialization
let sendgridInitialized = false;
if (config.sendgrid.enabled) {
  sendgridInitialized = sendGridService.isConfigured();
  if (sendgridInitialized) {
    logger.info('✅ SendGrid service initialized');
  } else {
    logger.warn('⚠️ SendGrid service not properly configured');
  }
} else {
  logger.warn('⚠️ SendGrid not enabled');
}

// RAG (Retrieval Augmented Generation) initialization
let ragInitialized = false;
if (config.chromadb.enabled && (config.openai.enabled || config.groq.enabled)) {
  ragInitialized = ragService.initialized;
  if (ragInitialized) {
    logger.info('✅ RAG (Retrieval Augmented Generation) service initialized');
  } else {
    logger.warn('⚠️ RAG service not properly configured (requires both ChromaDB and AI provider)');
  }
} else {
  logger.warn('⚠️ RAG service not enabled (requires both ChromaDB and AI provider)');
}
...
// Sentry error tracking middleware (should be first)
const sentryMiddlewares = sentryService.getSentryMiddleware();
if (sentryMiddlewares) {
...
app.get('/health', async (req, res) => {
  try {
    ...
    if (config.sendgrid.enabled) {
      sendgridStatus = sendGridService.isConfigured() ? 'configured' : 'error';
    }

    // Check RAG service
    let ragStatus = 'disabled';
    if (config.chromadb.enabled && (config.openai.enabled || config.groq.enabled)) {
      ragStatus = ragService.initialized ? 'connected' : 'error';
    }
    ...
    const health = {
      ...
      services: {
        ...
        posthog: posthogInitialized ? 'connected' : 'disabled',
        sentry: sentryInitialized ? 'connected' : 'disabled',
...
// Sentry error handler (should be after other error handlers)
const sentryErrorHandler = sentryService.getSentryErrorHandler();
...
const server = app.listen(config.port, '0.0.0.0', () => {
  ...
  if (config.posthog.enabled) {
    logger.info(`📊 PostHog:   ${config.posthog.host}`);
  }
  if (config.sentry.enabled) {
    logger.info(`🚨 Sentry:    Enabled`);
  }
  if (config.sendgrid.enabled) {
    logger.info(`📧 SendGrid:  ${config.sendgrid.fromEmail}`);
  }
  if (ragInitialized) {
