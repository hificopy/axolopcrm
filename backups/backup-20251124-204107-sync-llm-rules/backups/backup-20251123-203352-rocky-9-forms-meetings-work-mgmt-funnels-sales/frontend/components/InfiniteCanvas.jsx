...
const InfiniteCanvas = ({
  ...
  const drawObject = useCallback((ctx, obj, isSelected) => {
    ...
    switch (obj.type) {
      case 'rectangle':
      case 'shape':
        ctx.fillStyle = obj.style?.fill || '#791C14';
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    if (selectedTool === 'select') {
      ...
      setNewObject({
        ...
        style: selectedTool === 'rectangle' ? { fill: '#791C14', rx: 10 } : selectedTool === 'circle' ? { fill: '#4C7FFF' } : {},
      });
    }
