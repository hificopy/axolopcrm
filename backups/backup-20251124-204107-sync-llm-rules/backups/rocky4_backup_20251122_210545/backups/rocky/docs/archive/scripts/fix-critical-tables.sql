-- =====================================================
-- FIX CRITICAL TABLES WITHOUT user_id
-- These tables have RLS enabled but no user_id column
-- This blocks ALL access - we need to fix immediately
-- =====================================================
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'affiliate_clicks' AND column_name = 'user_id') THEN
      ...
      IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'affiliate_clicks' AND column_name = 'affiliate_id') THEN
        ...
        CREATE POLICY "users_select_own_affiliate_clicks" ON public.affiliate_clicks
          FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL); -- Allow NULL for system-generated clicks
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = table_name) THEN
      ...
      IF has_user_id AND has_rls AND policy_count >= 1 THEN
        RAISE NOTICE '✅ %: user_id ✓, RLS ✓, % policies', table_name, policy_count;
      ELSIF has_rls AND policy_count >= 1 THEN
        RAISE NOTICE '⚠️  %: NO user_id (special case), RLS ✓, % policies', table_name, policy_count;
      ELSE
        RAISE NOTICE '❌ %: Still has issues!', table_name;
      END IF;
    ELSE
