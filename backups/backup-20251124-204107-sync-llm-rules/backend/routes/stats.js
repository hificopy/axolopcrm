...
router.get('/overview', authenticateUser, async (req, res) => {
  try {
    ...
    logger.error('Error fetching stats overview', { error: error.message, stack: error.stack });
    res.status(500).json({
...
router.get('/sales', authenticateUser, async (req, res) => {
  try {
    ...
    const pipelineValue = (opportunitiesResult.data || [])
...
router.get('/leads', authenticateUser, async (req, res) => {
  try {
    ...
    const totalValue = (valueResult.data || [])
...
router.get('/opportunities', authenticateUser, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      winRate: (totalResult.count || 0) > 0
        ? Math.round(((wonResult.count || 0) / (totalResult.count || 0)) * 100)
        : 0
    };
