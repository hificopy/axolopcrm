...
class UserPreferencesService {
  ...
  async updatePinnedQuickActions(userId, pinnedButtons) {
    try {
      ...
      if (!existing) {
        // If no record exists, create one with the pinned actions
        const { data, error } = await supabase
          .from('user_preferences')
          .insert({
            user_id: userId,
        ...
        // If record exists, update just the pinned_quick_actions field in JSONB using raw SQL
        // This prevents race conditions between pinned actions and sidebar menu updates
        const { data, error } = await supabase
          .from('user_preferences')
          .update({
            preferences: supabase.sql`preferences || '{"pinned_quick_actions": ${JSON.stringify(pinnedButtons)} }'::jsonb`,
            updated_at: new Date().toISOString()
          })
          .eq('user_id', userId)
          .select()
  ...
  async updateSidebarMenuButtons(userId, buttons) {
    try {
      ...
      if (!existing) {
        // If no record exists, create one with the sidebar menu buttons
        const { data, error } = await supabase
          .from('user_preferences')
          .insert({
            user_id: userId,
        ...
        // If record exists, update just the sidebar_menu_buttons field in JSONB using raw SQL
        // This prevents race conditions between pinned actions and sidebar menu updates
        const { data, error } = await supabase
          .from('user_preferences')
          .update({
            preferences: supabase.sql`preferences || '{"sidebar_menu_buttons": ${JSON.stringify(buttons)} }'::jsonb`,
            updated_at: new Date().toISOString()
          })
          .eq('user_id', userId)
          .select()
