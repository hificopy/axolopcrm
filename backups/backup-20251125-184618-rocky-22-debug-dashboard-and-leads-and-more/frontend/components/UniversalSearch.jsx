...
export const UniversalSearch = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("axolop-recent-commands");
    if (saved) {
      try {
  ...
  const handleSelectItem = useCallback(
    (item) => {
      ...
      if (item.action_type) {
        ...
        // Normal entity navigation
        navigate(`/app/${entityType}${item.id ? `/${item.id}` : ""}`);
      }
  ...
  const clearRecentCommands = () => {
    setRecentCommands([]);
    localStorage.removeItem("axolop-recent-commands");
  };
