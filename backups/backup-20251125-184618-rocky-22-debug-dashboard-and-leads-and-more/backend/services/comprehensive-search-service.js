...
class ComprehensiveSearchService {
  ...
  async searchNotifications(userId, query, limit = 8) {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .or(`title.ilike.%${query}%,message.ilike.%${query}%,entity_type.ilike.%${query}%`)
      .order('created_at', { ascending: false })
      .limit(limit);
  ...
  async searchQuickActions(query, limit = 8) {
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('enabled', true)
      .or(`title.ilike.%${query}%,description.ilike.%${query}%,target_entity.ilike.%${query}%`)
   ...
   */
  async searchApiIntegrations(userId, query, limit = 8) {
    if (!userId) return [];
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('agency_id', agencyId)
      .or(`service_name.ilike.%${query}%,integration_type.ilike.%${query}%`)
      .limit(limit);
    
    if (error) {
      console.error('Error searching API integrations:', error);
      return [];
    }
    ...
    return data?.map(item => ({
      ...
      subtitle: `${item.integration_type} • ${item.status}`,
      status: item.status,
   ...
   */
  async searchCommandHistory(userId, query, limit = 8) {
    if (!userId) return [];
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .or(`command.ilike.%${query}%,entity_type.ilike.%${query}%`)
      .order('created_at', { ascending: false })
      .limit(limit);
    ...
    return data?.map(item => ({
      ...
      name: item.command,
      title: item.command,
      subtitle: `${item.command_type} • ${new Date(item.created_at).toLocaleString()}`,
   ...
   */
  async searchUserActivity(userId, query, limit = 8) {
    if (!userId) return [];
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .or(`action.ilike.%${query}%,entity_type.ilike.%${query}%,page_url.ilike.%${query}%`)
      .order('created_at', { ascending: false })
      .limit(limit);
    ...
    return data?.map(item => ({
      ...item,
      type: 'user_activity',
      name: `${item.action} - ${item.entity_type || 'Page'}`,
      title: `${item.action} - ${item.entity_type || 'Page'}`,
      subtitle: new Date(item.created_at).toLocaleString(),
      action: item.action,
      category: 'activity'
    })) || [];
   ...
   */
  async searchSystemStatus(query, limit = 8) {
    const { data, error } = await supabase
      .from('system_status')
      .select('*')
      .or(`service_name.ilike.%${query}%,status.ilike.%${query}%,message.ilike.%${query}%`)
      .order('last_checked', { ascending: false })
      .limit(limit);
    
    if (error) {
      console.error('Error searching system status:', error);
      return [];
    }
    ...
    return data?.map(item => ({
      ...
      name: item.service_name,
      title: item.service_name,
      subtitle: `${item.status} • ${item.message || 'No message'}`,
      status: item.status,
      category: 'system'
    })) || [];
  }
   ...
   */
  async logCommandExecution(userId, command, commandType, entityType = null, entityId = null, success = true, executionTime = 0, errorMessage = null) {
    if (!userId) return;
  ...
  /**
   * Enhanced Search with All New Entities
   */
  async searchAll(userId, query, options = {}) {
    ...
    // If no userId, only search navigation and help
    if (!userId) {
      const [navigationPages, helpArticles, quickActions] = await Promise.all([
        this.searchNavigationPages(searchQuery),
        this.searchHelpArticles(searchQuery, limit),
        this.searchQuickActions(searchQuery, limit)
      ]);

      return {
        query,
        results: [...navigationPages, ...helpArticles, ...quickActions],
        totalCount: navigationPages.length + helpArticles.length + quickActions.length,
        categories: {
          navigation: navigationPages.length,
          help_articles: helpArticles.length,
          quick_actions: quickActions.length,
        },
      };
    }
    ...
    // Execute ALL searches in parallel for maximum speed
    const [
      // Core CRM Data (existing)
      leads,
      contacts,
      opportunities,
      activities,

      // Communication & Engagement (existing)
      emailCampaigns,
      inboxItems,
      callLogs,
      conversationHistory,

      // Automation & Workflows (existing)
      workflows,
      workflowTemplates,
      workflowExecutions,
      automationTriggers,

      // Marketing & Forms (existing)
      forms,
      formSubmissions,
      ...
      // Settings & Configuration
      this.searchUserPreferences(userId, searchQuery, limit),
      this.searchAppSettings(userId, searchQuery, limit),
      this.searchFeatureFlags(userId, searchQuery, limit),

      // System & Audit
      this.searchAuditLogs(userId, searchQuery, limit),
      this.searchNotifications(userId, searchQuery, limit),
      this.searchSystemStatus(searchQuery, limit),

      // Help & Documentation
      this.searchHelpArticles(searchQuery, limit),
      this.searchVideoTutorials(searchQuery, limit),

      // Quick Actions & Commands
      this.searchQuickActions(searchQuery, limit),
      this.searchCommandHistory(userId, searchQuery, limit),

      // Integrations
      this.searchApiIntegrations(userId, searchQuery, limit),

      // User Activity
      this.searchUserActivity(userId, searchQuery, limit),
    ...
    const allResults = [
      ...
      ...emailCampaigns, ...inboxItems, ...callLogs, ...conversationHistory,
      ...workflows, ...workflowTemplates, ...workflowExecutions, ...automationTriggers,
      ...forms, ...formSubmissions,
      ...secondBrainNodes, ...secondBrainMaps, ...secondBrainNotes, ...secondBrainFolders,
      ...calendarEvents, ...importantDates, ...recurringEvents,
      ...savedReports,
      ...userPreferences, ...appSettings, ...featureFlags,
      ...auditLogs, ...notifications, ...systemStatus,
      ...helpArticles, ...videoTutorials,
      ...quickActions, ...commandHistory,
      ...apiIntegrations,
      ...userActivity,
      ...navigationPages,
    ];
    ...
    const categories = {
      ...
      workflow_executions: workflowExecutions.length,
      automation_triggers: automationTriggers.length,
      
      // Second Brain
      nodes: secondBrainNodes.length,
      maps: secondBrainMaps.length,
      notes: secondBrainNotes.length,
      folders: secondBrainFolders.length,
      ...
      // Reports
      saved_reports: savedReports.length,
      ...
      // Help & Documentation
      help_articles: helpArticles.length,
      video_tutorials: videoTutorials.length,
      
      // Quick Actions & Commands
      quick_actions: quickActions.length,
      command_history: commandHistory.length,
      
      // Integrations
      api_integrations: apiIntegrations.length,
      
      // User Activity
      user_activity: userActivity.length,
