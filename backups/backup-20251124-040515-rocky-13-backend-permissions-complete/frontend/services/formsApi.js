...
// Token refresh mutex to prevent race conditions across tabs
  ...
  async makeAuthenticatedRequest(url, options = {}) {
    ...
    try {
      ...
      if (response.status === 401 && !options._retry) {
        ...
        if (isRefreshing) {
          console.log('🔄 [FormsAPI] Already refreshing, waiting for existing refresh...');
          await refreshPromise;
        } else {
          isRefreshing = true;
          console.log('🔄 [FormsAPI] Starting token refresh...');
          ...
          refreshPromise = supabase.auth.refreshSession()
            .then(({ error: refreshError }) => {
              ...
              console.log('✅ [FormsAPI] Token refresh successful');
            })
            .finally(() => {
              isRefreshing = false;
              refreshPromise = null;
        ...
        console.log('🔵 [FormsAPI] Retrying with refreshed token...');
