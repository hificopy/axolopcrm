...
class RequestQueue {
   ...
   */
  shouldQueue() {
    // Use imported singleton for reliability
    const singleton = supabaseSingleton || window.supabaseSingleton;

    // Queue if token refresh is in progress
    if (singleton?.isRefreshing) {
      return true;
    }
  ...
  async handleAuthError(error, queuedRequest) {
    ...
    try {
      // Use the imported singleton, not window (for reliability)
      const singleton = supabaseSingleton || window.supabaseSingleton;

      if (!singleton) {
        console.error("❌ Supabase singleton not available for token refresh");
        queuedRequest.reject(new Error("Auth service not initialized"));
        return;
      }

      // Trigger token refresh if not already in progress
      if (!singleton.isRefreshing) {
        await singleton.refreshToken();
      }
      ...
      const session = await singleton.getSession();
      if (session?.access_token) {
