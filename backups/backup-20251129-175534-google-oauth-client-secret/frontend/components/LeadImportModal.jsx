...
const LeadImportModal = ({ children }) => {
    ...
    reader.onload = (e) => {
      ...
      if (lines.length > 0) {
        ...
        headers.forEach((header) => {
          const crmField = CRM_FIELDS.find(
  ...
  const handleAddCustomField = () => {
    setCustomFields((prev) => [
      ...prev,
      { id: prev.length + 1, name: `Custom ${prev.length + 1}` },
    ]);
  };
  ...
  const handleLoadPreset = (presetId) => {
    ...
    if (preset) {
      ...
      const newCustomFields = Object.entries(preset.mapping)
        .filter(
          ([csvHeader, crmField]) =>
            !CRM_FIELDS.some((f) => f.key === crmField),
        )
        .map(([csvHeader, crmField], index) => ({
          id: index + 1,
          name: csvHeader,
        }));
          ...
          {selectedSource === "custom" && csvHeaders.length > 0 && (
            <div className="mt-4 border p-4 rounded-md">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Map your CSV columns to CRM fields. Unmapped columns will be
              ...
              <div className="mt-4 pt-4 border-t">
                ...
                <p className="text-sm text-gray-500 mb-2">
                  These will be automatically detected if not mapped above. You
                  can also define them here.
                </p>
                {customFields.map((field) => (
                  <div key={field.id} className="flex items-center gap-2 mb-2">
                    <Input
                      value={field.name}
                      onChange={(e) =>
                        handleCustomFieldNameChange(field.id, e.target.value)
                      }
              ...
              <div className="mt-4 pt-4 border-t">
                <h4 className="text-md font-semibold mb-2">
                  Spreadsheet Preview (Conceptual)
                </h4>
