...
const updateLead = async (userId, leadId, leadData, agencyId = null) => {
  ...
  if (leadData.status && leadData.status !== currentLead.status) {
    try {
      ...
      console.log(
        `Workflow processed for status change: ${JSON.stringify(workflowResult)}`,
      );
    } catch (workflowError) {
...
const importLeadsFromCsv = async (
  ...
  const validation = csvService.validateCSV(
    parseResult.data,
    parseResult.headers,
  );
  ...
  if (presetId) {
    ...
    const suggestions = csvService.generateMappingSuggestions(
      parseResult.headers,
    );
    mapping = {};
