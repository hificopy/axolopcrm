...
import { Filter, Download, History as HistoryIcon, UserPlus, Mail, Phone, DollarSign, Edit, Trash2, CheckCircle, XCircle, Clock, FileText, Calendar } from 'lucide-react';
...
const EVENT_TYPES = {
  ...
  LEAD_DELETED: { icon: Trash2, label: 'Lead Deleted', color: 'bg-red-500' },
  OPPORTUNITY_CREATED: { icon: DollarSign, label: 'Opportunity Created', color: 'bg-green-500' },
  OPPORTUNITY_UPDATED: { icon: Edit, label: 'Opportunity Updated', color: 'bg-yellow-500' },
  OPPORTUNITY_WON: { icon: CheckCircle, label: 'Opportunity Won', color: 'bg-emerald-500' },
  OPPORTUNITY_LOST: { icon: XCircle, label: 'Opportunity Lost', color: 'bg-red-500' },
  ...
  STATUS_CHANGED: { icon: Edit, label: 'Status Changed', color: 'bg-orange-500' },
  MEETING_BOOKED: { icon: Calendar, label: 'Meeting Booked', color: 'bg-cyan-500' },
...
export default function History() {
  ...
  useEffect(() => {
    ...
    if (filterDateRange !== 'ALL') {
      ...
      if (startDate) {
        filtered = filtered.filter(event => new Date(event.created_at) >= startDate);
      }
    }
  ...
  const handleExport = () => {
    try {
      const csvHeaders = ['Date', 'Event Type', 'Title', 'Description', 'Entity', 'User'];
        ...
        <div className="crm-stats-grid mt-6">
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl p-5 border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Total Events</div>
                <div className="text-3xl font-bold text-gray-900 mt-1">{stats.totalEvents}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-5 border border-emerald-200 shadow-md hover:shadow-lg transition-all duration-300">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-semibold text-emerald-700 uppercase tracking-wide">Today</div>
                <div className="text-3xl font-bold text-emerald-600 mt-1">{stats.todayEvents}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-5 border border-amber-200 shadow-md hover:shadow-lg transition-all duration-300">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-3xl font-bold text-amber-600 mt-1">{stats.thisWeekEvents}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-5 border border-[#791C14]/30 shadow-md hover:shadow-lg transition-all duration-300">
            <div className="flex items-center gap-2">
              <div className="p-2 rounded-lg bg-[#791C14]/10">
                <Clock className="h-5 w-5 text-[#791C14]" />
              </div>
              <div>
                <div className="text-xs font-semibold text-[#791C14] uppercase tracking-wide">This Month</div>
                <div className="text-3xl font-bold text-[#791C14] mt-1">{stats.thisMonthEvents}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-6 bg-gray-50">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <HistoryIcon className="h-16 w-16 text-[#791C14]/30 mb-4" />
            ...
            {Object.entries(groupedEvents).map(([date, events]) => (
              <div key={date} className="mb-8">
                ...
                <div className="space-y-3 relative before:absolute before:left-6 before:top-0 before:bottom-0 before:w-0.5 before:bg-gray-200">
                  {events.map((event) => (
                    <Card key={event.id} className="ml-12 hover:shadow-md transition-shadow">
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                ...
                                {event.event_type === 'MEETING_BOOKED' && event.metadata && (
                                  <div className="mt-3 p-3 bg-cyan-50 border border-cyan-200 rounded-lg space-y-2">
                                    {event.metadata.scheduled_time && (
                                      <div className="flex items-center gap-2 text-sm">
                                        ...
                                        <span className="font-medium text-cyan-900">
                                          {new Date(event.metadata.scheduled_time).toLocaleString('en-US', {
                                            weekday: 'short',
                                            month: 'short',
                                            day: 'numeric',
                                            hour: 'numeric',
                                        ...
                                        {event.metadata.duration && (
                                          <span className="text-cyan-700">({event.metadata.duration} min)</span>
                                        )}
                                      </div>
                                    )}
                                    <div className="flex flex-wrap gap-3 text-xs text-cyan-800">
                                      ...
                                      {event.metadata.company && (
                                        <div className="flex items-center gap-1">
                                          <span className="font-medium">{event.metadata.company}</span>
                                        </div>
                                      )}
                            ...
                            <div className="flex items-center gap-4 text-xs text-gray-500 mt-3">
                              {event.entity_name && event.event_type !== 'MEETING_BOOKED' && (
                                <div className="flex items-center gap-1">
