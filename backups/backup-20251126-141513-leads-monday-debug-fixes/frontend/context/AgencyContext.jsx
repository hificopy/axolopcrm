...
export const AgencyProvider = ({ children }) => {
  const { user, supabase, loading: supabaseLoading, isInitialized } = useSupabase();
  ...
  const loadAgencies = useCallback(async () => {
    ...
    try {
      ...
      console.log("[AgencyContext] Loading agencies for user:", user.id, "attempt:", currentAttempt);
  ...
  useEffect(() => {
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      if (loading) {
        console.warn('[AgencyContext] Loading timeout reached - forcing completion');
        setLoading(false);
