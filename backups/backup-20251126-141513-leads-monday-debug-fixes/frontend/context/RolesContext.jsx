...
export function RolesProvider({ children }) {
  ...
  const fetchRoleTemplates = useCallback(async () => {
    try {
      const response = await api.get('/api/v1/roles/templates');
  ...
  const fetchTemplateCategories = useCallback(async () => {
    try {
      const response = await api.get('/api/v1/roles/templates/categories');
      if (response.data.success) {
  ...
  const fetchAgencyRoles = useCallback(async () => {
    ...
    try {
      const response = await api.get('/api/v1/roles/agency', {
  ...
  const fetchMyPermissions = useCallback(async () => {
    ...
    try {
      const response = await api.get('/api/v1/roles/my-permissions', {
        headers: { 'X-Agency-ID': currentAgency.id }
      });
      if (response.data.success) {
  ...
  useEffect(() => {
    ...
    loadData();
  }, [currentAgency?.id, fetchRoleTemplates, fetchTemplateCategories, fetchAgencyRoles, fetchMyPermissions]);

  // Permission check helpers
  const hasPermission = useCallback((permission) => {
    // Owner and admin have all permissions (except billing for admin)
    if (myMemberType === 'owner') return true;
    if (myMemberType === 'admin' && permission !== 'can_manage_billing') return true;
  ...
  const copyTemplateToAgency = async (templateId) => {
    ...
    const response = await api.post('/api/v1/roles/agency/copy-template', {
  ...
  const createAgencyRole = async (roleData) => {
    ...
    const response = await api.post('/api/v1/roles/agency', roleData, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const updateAgencyRole = async (roleId, updates) => {
    ...
    const response = await api.put(`/api/v1/roles/agency/${roleId}`, updates, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const deleteAgencyRole = async (roleId) => {
    ...
    const response = await api.delete(`/api/v1/roles/agency/${roleId}`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const getMemberRoles = async (memberId) => {
    ...
    const response = await api.get(`/api/v1/roles/member/${memberId}`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const assignRoleToMember = async (memberId, roleId) => {
    ...
    const response = await api.post(`/api/v1/roles/member/${memberId}/assign`, {
      role_id: roleId
    }, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const removeRoleFromMember = async (memberId, roleId) => {
    ...
    const response = await api.delete(`/api/v1/roles/member/${memberId}/roles/${roleId}`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const setMemberRoles = async (memberId, roleIds) => {
    ...
    const response = await api.put(`/api/v1/roles/member/${memberId}/roles`, {
      role_ids: roleIds
    }, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const getMemberOverrides = async (memberId) => {
    ...
    const response = await api.get(`/api/v1/roles/member/${memberId}/overrides`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const setPermissionOverride = async (memberId, permissionKey, value, reason = null) => {
    if (!currentAgency?.id) throw new Error('No agency selected');

    const response = await api.post(`/api/v1/roles/member/${memberId}/overrides`, {
  ...
  const removePermissionOverride = async (memberId, permissionKey) => {
    ...
    const response = await api.delete(`/api/v1/roles/member/${memberId}/overrides/${permissionKey}`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
  ...
  const getMemberEffectivePermissions = async (memberId) => {
    ...
    const response = await api.get(`/api/v1/roles/member/${memberId}/permissions`, {
      headers: { 'X-Agency-ID': currentAgency.id }
    });
