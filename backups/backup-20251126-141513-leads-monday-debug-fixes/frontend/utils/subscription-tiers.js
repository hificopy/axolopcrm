...
export function getPricingComparison(
  ...
  const fromPrice =
    billingCycle === BILLING_CYCLES.YEARLY ? from.yearly : from.monthly * 12;
  const toPrice =
    billingCycle === BILLING_CYCLES.YEARLY ? to.yearly : to.monthly * 12;
...
// Section access checking function (for navigation/search)
export function canAccessSection(userTier, section) {
  if (!userTier || userTier === "god_mode") return true;
  ...
  const allowedSections = sectionAccess[userTier] || [];
  // If section not in any tier list, allow it by default (public sections)
  if (!Object.values(sectionAccess).flat().includes(section)) return true;
