...
export const useSafeSupabase = () => {
  ...
  if (!context) {
    console.warn(
      "[useSafeSupabase] SupabaseContext not found - returning default values",
    );
    return {
      supabase: null,
      user: null,
...
export const SupabaseProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      if (loading) {
        console.warn(
          "[SupabaseContext] Loading timeout reached - forcing completion",
        );
        setLoading(false);
      }
    ...
    const initializeClient = async () => {
      ...
      if (
        ...
        console.warn(
          "[SupabaseContext] Supabase not configured - using mock client",
        );
        const mockClient = {
          auth: {
            signInWithOAuth: () =>
      ...
      try {
        ...
        if (initialSession) {
          ...
          setUser(initialUser);
          console.log(
            "[SupabaseContext] Initial session loaded:",
            initialUser?.id,
          );
        }
