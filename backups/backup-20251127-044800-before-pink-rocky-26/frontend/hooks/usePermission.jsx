...
export function usePermission(permission, mode = 'single') {
  ...
  if (Array.isArray(permission)) {
    // Empty array - no permissions to check
    if (permission.length === 0) {
      return false;
    }

    if (mode === 'any') {
      // Admin check for any: true if at least one permission isn't billing
      if (isAdmin) {
        return permission.some(p => p !== 'can_manage_billing');
      }
      return hasAnyPermission(permission);
    }

    if (mode === 'all') {
      // Admin check for all: true only if NONE of the permissions are billing
      if (isAdmin) {
