...
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://fuclpfhitgwugxogxkmw.supabase.co';
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || 'sb_secret_Y0pdE6KRUTcbDmFe1lqWFw_JapIHko3';
...
async function createTestUser() {
  ...
  try {
    ...
    if (existingUser && !existingUserError) {
      ...
      console.log(`🔄 This user will always go through onboarding every time they log out`);
      return;
    }
    ...
    const { data: userProfile, error: profileError } = await supabase
      ...
      .insert([{
        ...
        first_name: 'Test',
        last_name: 'User',
        onboarding_completed: false, // This ensures the user always goes through onboarding
        email_confirmed: true
      }])
      .select()
      .single();

    if (profileError && profileError.code !== '23505') { // 23505 is unique violation, which we can ignore
