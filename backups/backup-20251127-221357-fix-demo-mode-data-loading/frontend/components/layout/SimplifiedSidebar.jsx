...
import {
  Home, DollarSign, Users, Send, Headset, Inbox, UserPlus, Target,
...
const advancedCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Reports', href: '/app/reports', icon: BarChart3, locked: true, version: 'V1.1' },
    ]
  },
  {
    name: 'Teams',
...
export default function SimplifiedSidebar({ isSidebarCollapsed, setIsSidebarCollapsed, pinnedButtons, onPinnedChange, currentTheme }) {
  ...
  const { currentAgency, getCurrentTier, isGodMode, isFeatureEnabled } = useAgency();
  ...
  const isSectionAccessible = (sectionKey) => {
    ...
    const tierFeatures = {
      sales: ['home', 'leads', 'contacts', 'calendar', 'tasks', 'pipeline', 'opportunities', 'activities', 'conversations', 'calls', 'inbox'],
      build: ['home', 'leads', 'contacts', 'calendar', 'tasks', 'pipeline', 'opportunities', 'activities', 'conversations', 'calls', 'inbox', 'forms', 'email_marketing', 'automation', 'reports', 'second_brain'],
      scale: ['home', 'leads', 'contacts', 'calendar', 'tasks', 'pipeline', 'opportunities', 'activities', 'conversations', 'calls', 'inbox', 'forms', 'email_marketing', 'automation', 'reports', 'second_brain', 'white_label', 'api', 'mind_maps'],
    };
    ...
    <>
      ...
      <div
        className={`
          ...
          flex flex-col shadow-2xl z-50 transition-all duration-300 ease-out
          ${isSidebarCollapsed ? 'w-6' : 'w-64'}
      ...
      >
        {/* Header - Always shows logo and text */}
        <div className="h-16 bg-transparent flex items-center px-4 flex-shrink-0">
          <div className="flex items-center gap-2 -mt-[32px]">
            ...
            />
            <span className="text-white font-bold text-lg whitespace-nowrap">Axolop</span>
          </div>
        </div>

        {/* Collapse/Expand Button - Right Edge, Vertically Centered */}
        <button
              ...
              {coreNavigation.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && !isLocked && !isSidebarCollapsed && (
                    ...
                    return (
                      <div key={category.name} className="relative group">
                        <button
                          ...
                          className="w-full flex items-center justify-center p-3 rounded-lg text-gray-300 hover:bg-white/5 hover:text-white transition-all duration-200"
                        >
                      ...
                      >
                        <div className="flex items-center flex-1 px-3 py-2">
                          <div className={`p-2 rounded-lg mr-3 transition-colors ${
                            isExpanded ? 'bg-white/20 text-white' : 'text-gray-300'
                          }`}>
                              ...
                              if (item.locked) {
                                return (
                                  <div key={item.name} className="relative group ml-2">
                                    <div className="flex items-center justify-between py-2 px-3 rounded-lg cursor-not-allowed text-gray-300 bg-gray-800/30 border border-gray-700/40">
                                      <div className="flex items-center">
                                        <ItemIcon className="h-4 w-4 mr-3 text-gray-300" />
                                ...
                                <div key={item.name}>
                                  <div
                                    className={`flex items-center justify-between py-2 px-3 rounded-lg cursor-pointer ml-2 ${
                                      isActive ? 'text-white bg-white/10' : 'text-gray-300 hover:bg-gray-500/30'
                                    }`}
                                    onClick={() => {
                                      ...
                                      {item.subItems.map((subItem) => {
                                        ...
                                        return subItem.locked ? (
                                          <div key={subItem.name} className="relative group ml-2">
                                            <div className="flex items-center py-2 px-3 rounded-lg cursor-not-allowed text-gray-300 bg-gray-800/30">
                                              <SubItemIcon className="h-3.5 w-3.5 mr-3 text-gray-300" />
                                              <span className="text-xs text-gray-300">{subItem.name}</span>
                                          ...
                                          <Link
                                            ...
                                            className={`flex items-center py-2 px-3 rounded-lg transition-all ${
                                              isSubActive ? 'bg-white/10 text-white' : 'text-gray-300 hover:bg-gray-500/30'
                                            }`}
                                          >
                            ...
                            if (item.betaOnly) {
                              ...
                              if (!hasAccess) {
                                return (
                                  ...
                                  >
                                    <div className="flex items-center justify-between py-2 px-3 rounded-lg text-gray-300 bg-gray-800/30 border border-yellow-500/40 hover:border-yellow-500/60">
                            ...
                            return item.locked ? (
                              ...
                              <Link
                                ...
                                className={`flex items-center justify-between py-2 px-3 rounded-lg transition-all ml-2 ${
                                  isActive ? 'bg-white/10 text-white' : 'text-gray-300 hover:bg-gray-500/30'
                                }`}
                              >
                                <div className="flex items-center">
                                  <ItemIcon className="h-4 w-4 mr-3" />
