...
const filterOptions = [
  ...
  { id: "deals", label: "Opportunities", icon: TrendingUp },
...
const categoryConfig = {
  leads: {
    icon: UserPlus,
    label: "Leads",
    color: "from-[#3F0D28] via-[#5a1a3a] to-[#4a1530]",
    glow: "shadow-[0_0_20px_rgba(63,13,40,0.4)]",
...
export default function UltraSmoothMasterSearch({ isOpen, onClose }) {
    ...
    try {
      ...
      const filterParams =
        selectedFilters.length > 0
          ...
          : "";
      const url = `/api/search?q=${encodeURIComponent(searchQuery)}&limit=10${filterParams}`;
  ...
  const queryAI = useCallback(async (question) => {
    try {
      ...
      const response = await fetch(
        `/api/ai-assistant/query?q=${encodeURIComponent(question)}`,
      );
      const data = await response.json();
  ...
  const handleSelectResult = useCallback(
    (result) => {
      ...
      // Use the URL from the enhanced search result
      if (result.url) {
        navigate(result.url, {
          state: {
            fromSearch: true,
        ...
        console.error("No URL available for search result:", result);
      }
    ...
    (e) => {
      if (e.key === "ArrowDown") {
        ...
        if (results.length > 0 && results[selectedIndex]) {
          ...
          console.log(
            "No result selected, results length:",
            results.length,
            "selectedIndex:",
            selectedIndex,
          );
    ...
    (result, action) => {
      ...
      switch (action) {
        ...
        case "Send Email":
          ...
          navigate(
            `/inbox?compose=true&to=${result.metadata?.email || result.title}`,
          );
          onClose();
          break;
        ...
        case "Ask AI":
          ...
          navigate(
            `/ai-assistant?context=${result.category}&id=${result.id}&query=${encodeURIComponent(result.title)}`,
          );
          onClose();
          break;
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Premium Backdrop */}
        <div className="absolute inset-0 bg-gradient-to-br from-black/85 via-[#3F0D28]/15 to-black/85 backdrop-blur-xl" />
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <motion.div
            ...
            className="absolute top-0 left-1/4 w-96 h-96 bg-[#3F0D28]/20 rounded-full blur-[120px]"
          />
        </div>
        ...
        <motion.div
          ...
          style={{
            background:
              "linear-gradient(180deg, #2a0919 0%, #3F0D28 30%, #2a0919 70%, #1a0812 100%)",
            backdropFilter: "blur(40px)",
            border: "1px solid rgba(63, 13, 40, 0.5)",
            boxShadow:
              "inset 0 2px 4px rgba(255,255,255,0.15), inset 0 -2px 4px rgba(0,0,0,0.2), 0 25px 50px rgba(0, 0, 0, 0.4)",
          }}
        >
          ...
          <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-[#3F0D28] to-transparent" />

          {/* Header */}
          <div className="relative">
            <div
              className="flex items-center gap-4 p-5 border-b border-white/10"
            ...
            >
              ...
              >
                <Search
                  className={`h-6 w-6 transition-colors duration-200 ${loading ? "text-[#3F0D28]/60" : "text-[#3F0D28]"}`}
                />
                {!loading && (
                  <div className="absolute inset-0 bg-[#3F0D28]/20 blur-xl" />
                )}
              </motion.div>
              <input
                ref={inputRef}
          ...
          <div className="px-6 py-3 border-b border-white/10">
            <div className="flex items-center justify-between mb-3">
              ...
              >
                ...
                Filters
                {selectedFilters.length > 0 && (
                  <span className="px-2 py-0.5 bg-[#3F0D28] text-white text-xs rounded-full">
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${
                        isSelected
                          ? "bg-[#3F0D28] text-white border border-[#5a1a3a]"
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(13, 15, 18, 0.8) 0%, rgba(10, 12, 15, 0.9) 100%)",
          ...
          >
            <style>{`
              ...
              div::-webkit-scrollbar-thumb {
                background: rgba(63, 13, 40, 0.6);
                border-radius: 3px;
                transition: background 0.2s;
              }
              div::-webkit-scrollbar-thumb:hover {
                background: rgba(63, 13, 40, 0.8);
              }
            `}</style>
            ...
            <AnimatePresence mode="wait">
              {results.length === 0 && !loading && query.length >= 2 && (
                ...
                >
                  <div className="relative mb-6">
                    <div className="rounded-full bg-gradient-to-br from-[#3F0D28]/30 to-[#3F0D28]/10 p-6 backdrop-blur-xl border border-[#3F0D28]/30">
                      <Search className="h-12 w-12 text-[#3F0D28]" />
                    </div>
                    <motion.div
                      animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }}
                      transition={{ duration: 2, repeat: Infinity }}
                      className="absolute inset-0 bg-[#3F0D28]/20 blur-2xl rounded-full"
                    />
                  </div>
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-300">
                      Press{" "}
                      <kbd className="px-2 py-0.5 mx-1 rounded bg-white/10 text-xs font-mono border border-white/20">
                        Enter
                      </kbd>{" "}
                      to ask AI
                    </span>
              ...
              {results.length === 0 && !loading && query.length < 2 && (
                ...
                >
                  <div className="relative mb-6">
                    <div className="rounded-full bg-gradient-to-br from-[#3F0D28]/30 to-[#3F0D28]/10 p-6 backdrop-blur-xl border border-[#3F0D28]/30">
                      <Sparkles className="h-12 w-12 text-[#3F0D28]" />
                    </div>
                    <motion.div
                  ...
                  <p className="text-gray-400 text-sm mb-8 max-w-md">
                    Leads, contacts, campaigns, workflows, calendar events,
                    ...
                    {Object.entries(categoryConfig)
                      ...
                      .map(([key, { label, color, glow }]) => (
                        <motion.span
                          ...
                          transition={{
                            delay:
                              0.05 * Object.keys(categoryConfig).indexOf(key),
                          }}
            ...
            <AnimatePresence mode="wait">
              ...
              {aiResponse && (
                ...
                >
                  ...
                  {/* Suggested Actions */}
                  {aiResponse.response?.actions &&
                    aiResponse.response.actions.length > 0 && (
                      <div className="mb-4">
            ...
            <AnimatePresence mode="wait">
              {Object.entries(groupedResults).map(
                ([category, items], catIndex) => {
                  const config =
                    categoryConfig[category] || categoryConfig.navigation;
                          ...
                          >
                            <button
                              ...
                              className={`w-full flex items-start gap-4 px-5 py-4 transition-all duration-150 ${
                                isSelected
                                  ? "bg-gradient-to-r from-[#3F0D28]/20 to-transparent border-l-2 border-[#3F0D28]"
                              ...
                              >
                                {config.actions.slice(0, 3).map((action) => (
                                  <motion.button
                                    ...
                                    className="px-3 py-1 text-xs bg-white/10 hover:bg-[#3F0D28]/30 text-white rounded-lg backdrop-blur-sm border border-white/10 transition-colors"
                                  >
                                    {action}
                                  </motion.button>
                                ))}
          ...
          {results.length > 0 && (
            <motion.div
              ...
              style={{
                background:
                  "linear-gradient(180deg, rgba(13, 15, 18, 0.95) 0%, rgba(10, 12, 15, 0.98) 100%)",
                backdropFilter: "blur(20px)",
              }}
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500">
                  {Object.keys(categories).length} categor
                  {Object.keys(categories).length !== 1 ? "ies" : "y"}
                </span>
              </div>
            </motion.div>
          )}
          ...
          {/* Bottom Glow */}
          <div className="absolute bottom-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#3F0D28]/50 to-transparent" />
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
