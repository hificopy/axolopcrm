...
export default function UnifiedPipeline() {
  ...
  const pipelineStages = [
    ...
    { id: "qualified", name: "Qualified", color: "#1A777B", probability: 40, order: 2 },
    { id: "proposal", name: "Proposal", color: "#F59E0B", probability: 70, order: 3 },
    { id: "negotiation", name: "Negotiation", color: "#F97316", probability: 85, order: 4 },
  ...
  const metrics = {
    ...
    totalValue: filteredOpportunities.reduce((sum, opp) => sum + (opp.amount || 0), 0),
    weightedValue: filteredOpportunities.reduce((sum, opp) => sum + ((opp.amount || 0) * (opp.probability || 0) / 100), 0),
    byStage: pipelineStages.reduce((acc, stage) => {
      acc[stage.id] = filteredOpportunities.filter((opp) => opp.stage === stage.id).length;
      return acc;
    }, {}),
  ...
  const opportunityContextMenuConfig = (opportunity) => CRMMenuConfigs.deal(opportunity, {
      ...
      {pipelineStages.map((stage) => (
        <div key={stage.id} className="flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900" style={{ color: stage.color }}>
              {stage.name}
            </h3>
          ...
          <div className="flex-1 min-h-[400px] bg-gray-50 rounded-lg p-4 space-y-3">
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <Badge variant={opportunity.stage === 'won' ? 'success' : opportunity.stage === 'lost' ? 'destructive' : 'default'}>
                        {opportunity.stage}
                      </Badge>
                    </div>
                    
                    <p className="text-sm text-gray-600 mb-2">{opportunity.lead?.name} - {opportunity.lead?.company}</p>
            ...
            {filteredOpportunities.filter((opp) => opp.stage === stage.id).length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                {canCreate() && (
                  <Button
                    ...
                    size="sm"
                    className="mt-4"
                    onClick={() => {/* Handle create new opportunity */}}
                  >
            ...
            {filteredOpportunities.map((opportunity) => (
              <tr
                ...
                onContextMenu={(e) => {
                  ...
                  import('@/components/ui/ContextMenuProvider').then(({ useContextMenu }) => {
              ...
              >
                <td className="p-3">
                  ...
                  <div className="text-sm text-gray-600">{opportunity.lead?.name} - {opportunity.lead?.company}</div>
                </td>
                <td className="p-3 text-right font-medium">{formatCurrency(opportunity.amount)}</td>
                ...
                <td className="p-3">
                  <Badge 
                    variant={
                      ...
                      opportunity.stage === 'lost' ? 'destructive' : 'default'
                    }
                  >
                    {opportunity.stage}
                  </Badge>
                </td>
                <td className="p-3 text-sm text-gray-600">{opportunity.owner_name || 'Unassigned'}</td>
                <td className="p-3 text-sm text-gray-600">{formatDate(opportunity.created_at)}</td>
                <td className="p-3">
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-[#3F0D28]">{formatCurrency(metrics.totalValue)}</div>
            <div className="text-sm text-gray-600 font-medium">Total Value</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-2xl font-bold text-green-600">{formatCurrency(metrics.weightedValue)}</div>
            <div className="text-sm text-gray-600 font-medium">Weighted Value</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm">
          ...
          <div className="p-6 pt-20">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600">{selectedOpportunity.lead?.name} - {selectedOpportunity.lead?.company}</p>
              </div>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-sm font-semibold text-gray-700 uppercase mb-3">Deal Details</h3>
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Amount:</span>
                    <span className="font-medium">{formatCurrency(selectedOpportunity.amount)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{selectedOpportunity.stage}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Expected Close:</span>
                    <span className="font-medium">{formatDate(selectedOpportunity.expected_close_date)}</span>
                  </div>
                </div>
              </div>
            </div>
