...
| 3002 | backend API (dev) | Main API server for development | Active |
...
2. **Backend API (dev)**: Uses port 3002 for development server
...
lsof -ti:3002  # For development backend
lsof -ti:4001  # For Docker backend

# Kill process using port
kill -9 $(lsof -ti:3002)  # For development backend
