...
class GoogleCalendarService {
  ...
  async saveUserTokens(userId, tokens) {
    try {
      ...
      try {
        ...
        console.log('Gmail tokens table may not exist yet, skipping Gmail token save');
      }

      console.log(`✅ Google integration complete for user ${userId} - Calendar & Gmail connected`);
      return calendarData;
    } catch (error) {
   ...
   */
  async getEvents(userId, { calendarId = 'primary', timeMin, timeMax, maxResults = 250 }) {
    try {
      const calendar = await this.getCalendarClient(userId);
    ...
    try {
      ...
      try {
        const { data: gmailTokens } = await supabase
          ...
          .eq('user_id', userId)
          .single();
        gmailConnected = gmailTokens !== null && gmailTokens.access_token !== null;
      } catch (error) {
