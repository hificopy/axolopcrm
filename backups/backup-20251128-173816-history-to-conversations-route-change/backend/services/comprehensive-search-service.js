...
class ComprehensiveSearchService {
  ...
  async searchContacts(userId, query, limit) {
    try {
      ...
      return data.map((contact) => ({
        id: contact.id,
        title: `${contact.first_name || ''} ${contact.last_name || ''}`.trim() || 'Unknown Contact',
  ...
  async searchOpportunities(userId, query, limit) {
    try {
      const { data, error } = await supabase
        .from("opportunities")
        .select("id, name, value, stage, probability, close_date, lead:lead_id(name)")
        .eq("user_id", userId)
        .or(
          `name.ilike.%${query}%,stage.ilike.%${query}%`,
        )
        .limit(limit);
  ...
  async searchForms(userId, query, limit) {
    try {
      const { data, error } = await supabase
        .from("forms")
        .select("id, title, description, is_active, total_responses, created_at")
        .eq("user_id", userId)
        .is("deleted_at", null)
      ...
      return data.map((form) => ({
        ...
        metadata: { is_active: form.is_active, responses: form.total_responses },
      }));
    } catch (error) {
