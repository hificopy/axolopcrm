...
import CRMWorkflowService from "./CRMWorkflowService.js";
...
// Get all leads for a user (with optional agency filtering)
const getLeads = async (userId, agencyId = null) => {
  let query = supabase.from("leads").select("*");

  // If agencyId provided, filter by agency (allows seeing all agency leads for admins via RLS)
  if (agencyId) {
    query = query.eq("agency_id", agencyId);
  } else {
    // Fallback to user-only filter for backwards compatibility
...
// Get a single lead by ID (with optional agency filtering)
const getLeadById = async (userId, leadId, agencyId = null) => {
  let query = supabase.from("leads").select("*").eq("id", leadId);

  // If agencyId provided, use agency filter (RLS will handle visibility)
  if (agencyId) {
    query = query.eq("agency_id", agencyId);
  } else {
    query = query.eq("user_id", userId);
...
// Create a new lead (with optional agency_id)
const createLead = async (userId, leadData, agencyId = null) => {
  ...
  // Build insert data with optional agency_id
  const insertData = {
    user_id: userId,
    ...processedLeadData,
  };
...
// Update an existing lead (with optional agency_id filtering)
const updateLead = async (userId, leadId, leadData, agencyId = null) => {
  // Get current lead data for workflow processing
  ...
  const workflowService = new CRMWorkflowService(supabase);

  // Process workflow for status changes
  if (leadData.status && leadData.status !== currentLead.status) {
    try {
      const workflowResult = await workflowService.processLeadStatusChange(
        leadId, 
        leadData.status, 
        currentLead, 
        userId, 
      ...
      console.log(`Workflow processed for status change: ${JSON.stringify(workflowResult)}`);
    } catch (workflowError) {
      console.error("Workflow processing error:", workflowError);
      // Continue with the update even if workflow fails
    }
  }
  ...
  // Use agency filter if provided (RLS handles visibility)
  if (agencyId) {
    query = query.eq("agency_id", agencyId);
  } else {
    query = query.eq("user_id", userId);
...
// Function to import leads from CSV with enhanced validation
const importLeadsFromCsv = async (
  ...
  const parseResult = csvService.parseCSV(csvText);
  ...
  const validation = csvService.validateCSV(parseResult.data, parseResult.headers);
  
  if (!validation.isValid) {
    const errorMessage = `CSV validation failed:\n${
      validation.errors.map(err => `Row ${err.row}: ${err.errors.join(', ')}`).join('\n')
    }\n\nMissing required fields: ${validation.missingRequiredFields.join(', ')}`;
    throw new Error(errorMessage);
  }
  
  // Generate mapping suggestions if no preset provided
  let mapping = {};
  if (presetId) {
    ...
    // Auto-generate mapping from headers
    const suggestions = csvService.generateMappingSuggestions(parseResult.headers);
    mapping = {};
    ...
    // Fallback to default mapping for missing fields
  ...
  const cleanedData = csvService.cleanData(parseResult.data);
  const leadsToInsert = [];
  ...
  for (const record of cleanedData) {
    const lead = {
      ...
      status:
        ...
        record["Status"],
      value:
        record[Object.keys(mapping).find((key) => mapping[key] === "value")] ||
        record["Value"],
      owner_email:
        record[
          Object.keys(mapping).find((key) => mapping[key] === "Owner Email")
        ] || record["Owner Email"],
    };
  ...
  return {
    ...
    statistics: {
      ...
      duplicateEmails: validation.duplicateEmails.length,
      warnings: validation.warnings,
      fieldsFound: parseResult.headers.length
    }
  };
};
  }
