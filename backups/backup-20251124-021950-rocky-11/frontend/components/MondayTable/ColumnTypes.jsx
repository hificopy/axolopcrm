...
function PriorityCell({ value, editable, onChange, columnKey }) {
  if (!editable) {
    ...
    const priorityConfig = {
      critical: { bg: '#791C14', text: 'white', label: 'Critical', icon: '⚠️' },
  ...
  if (isEditing) {
    return (
      <Input
        ...
        autoFocus
        className="h-8 px-2 text-sm border-[#791C14] focus:ring-[#791C14]"
...
function CommentsCell({ value, onClick }) {
  ...
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="absolute -top-1 -right-1 bg-[#791C14] text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
          {count > 9 ? '9+' : count}
        </span>
      )}
    </button>
