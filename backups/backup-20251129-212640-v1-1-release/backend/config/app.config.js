...
const config = {
  ...
  google: {
    clientId:
      ...
      "922878594880-j42sv7qr8n5iosgjmc960kad8l9812cf.apps.googleusercontent.com",
    clientSecret:
      process.env.GOOGLE_CLIENT_SECRET || "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",
    enabled:
      !!process.env.GOOGLE_CLIENT_ID &&
  ...
  sentry: {
    ...
    enabled:
      !!process.env.SENTRY_DSN && process.env.SENTRY_DSN !== "your_sentry_dsn",
  },

  // Workflow Engine
  workflow: {
    pollIntervalMs: parseInt(process.env.WORKFLOW_POLL_INTERVAL || "5000", 10),
    emailPollIntervalMs: parseInt(
      process.env.EMAIL_POLL_INTERVAL || "10000",
      10,
    ),
    schedulePollIntervalMs: parseInt(
      process.env.SCHEDULE_POLL_INTERVAL || "60000",
      10,
    ),
...
function validateConfig() {
  ...
  if (
    config.encryption.key === "default_key_change_in_production" &&
    config.env === "production"
  ) {
    warnings.push(
      "⚠️  WARNING: Using default encryption key in production! Please set ENCRYPTION_KEY",
    );
  }
