...
if (config.workflow.enabled) {
  ...
  // Start subscription cron jobs (grace period, trial expiration)
  try {
    const cron = await import("node-cron");
    const subscriptionCron = await import(
      "./services/subscription-grace-period-cron.js"
    );

    // Run daily at 2 AM
    cron.default.schedule("0 2 * * *", async () => {
      logger.info("🕐 Running daily subscription cron jobs");
      await subscriptionCron.runSubscriptionCronJobs();
    });

    logger.info("✅ Subscription cron jobs scheduled (daily at 2 AM)");

    // Run immediately on startup for testing (optional - comment out in production)
    if (config.env === "development") {
      logger.info("🧪 Running subscription cron jobs on startup (dev mode)");
      setTimeout(async () => {
        await subscriptionCron.runSubscriptionCronJobs();
      }, 5000); // Run after 5 seconds
    }
  } catch (error) {
    logger.warn("⚠️ Subscription cron jobs not available:", error.message);
  }
}
