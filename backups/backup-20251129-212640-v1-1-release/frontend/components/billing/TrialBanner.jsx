...
export default function TrialBanner({ className }) {
  ...
  const handleDismiss = () => {
    ...
    const dismissDuration =
      daysLeft <= 3 ? 4 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000; // 4 hours if urgent, else 24 hours
      ...
      className={cn(
        ...
        isCritical
          ...
          : isUrgent
            ...
            : "bg-gradient-to-r from-[#3F0D28] to-[#6B2020] text-white",
        className,
      )}
    >
      {/* Left: Trial info */}
      <div className="flex items-center gap-3">
        ...
        <span className="font-medium">
          {isCritical ? (
            ...
            <>
              {daysLeft} day{daysLeft !== 1 ? "s" : ""} left in your {tierName}{" "}
              trial
            </>
          ) : (
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            {isCritical ? (
              ...
              <Sparkles className="h-5 w-5 text-[#3F0D28]" />
            )}
            <h3
              className={cn(
                "font-semibold",
          ...
          >
            {isCritical ? (
              <>
                Your trial ends in {hoursLeft} hours. Your card will be
                automatically charged.
              </>
            ) : isUrgent ? (
              <>
                {daysLeft} day{daysLeft !== 1 ? "s" : ""} and {hoursLeft} hours
                remaining
              </>
            ) : (
          ...
          <div className="mb-4">
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.max(0, Math.min(100, ((14 - daysLeft) / 14) * 100))}%`,
                }}
              />
            </div>
      ...
      <div className="border-t border-gray-200 pt-4 mt-4">
        ...
        <ul className="text-sm text-gray-600 space-y-1">
          <li className="flex items-center gap-2">
            ...
            Your card will be charged ${subscription?.price_monthly || "0"}
            /month
          </li>
          <li className="flex items-center gap-2">
            <span className="w-1 h-1 rounded-full bg-gray-400" />
