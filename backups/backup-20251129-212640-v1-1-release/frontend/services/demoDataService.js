...
export const demoDataService = {
  // Dashboard
  getSalesMetrics: (timeRange = 'month') => {
    console.log(
      `[DemoDataService] Returning sales metrics for timeRange: ${timeRange}`,
    );
    const revenueData = generateRevenueData(timeRange);
    return Promise.resolve({
      ...DEMO_DASHBOARD_METRICS.sales,
      totalRevenue: revenueData.totalRevenue,
      revenueByPeriod: revenueData.periods,
    });
  },

  getProfitLossData: (timeRange = "month") => {
    console.log(
    ...
    return Promise.resolve({
      ...
      expenses,
      profit,
      netProfit: profit, // ProfitMarginWidget expects 'netProfit' property
      profitMargin: parseFloat(profitMargin),
      expenseBreakdown: {
        software: Math.round(expenses * 0.17),
        salaries: Math.round(expenses * 0.65),
        marketing: Math.round(expenses * 0.13),
        operations: Math.round(expenses * 0.05),
      },
    });
  },

  getMarketingMetrics: (timeRange = "month") => {
    console.log(
  ...
  getDashboardData: (timeRange = "month") => {
    ...
    return {
      ...
      forms: DEMO_DASHBOARD_METRICS.forms,
    };
  },
