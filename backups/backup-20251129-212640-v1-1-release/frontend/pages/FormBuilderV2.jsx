...
export default function FormBuilderV2() {
  ...
  const initializeWorkflowFromQuestions = useCallback(
    (questionsArray) => {
      const nodes = [];
      const edges = [];
    ...
    if (formId && formId !== "new" && !formId.startsWith("new-")) {
      formsApi
        ...
        .then((formData) => {
          ...
          if (
            formData.settings?.workflow_nodes &&
            formData.settings?.workflow_nodes.length > 0
          ) {
            setWorkflowNodes(formData.settings.workflow_nodes);
            setWorkflowEdges(formData.settings.workflow_edges || []);
            setEndings(
              formData.settings.endings || [
                {
                  id: "end-default",
                  title: "Thank you!",
                  message: "Your response has been recorded.",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-crm-border py-3 px-4 flex items-center gap-3 overflow-x-auto">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 border border-crm-border rounded-md">
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${form.status === "active" ? "translate-x-4" : "translate-x-1"}`}
              />
            </button>
          </div>
          ...
          <Button
            ...
            onClick={() => {
              if (
                ...
                navigate(`/app/forms/preview/${form.id}`);
              } else {
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === "connect" && <ConnectTab form={form} />}
        {activeTab === "share" && <ShareTab form={form} />}
        {activeTab === "results" && <ResultsTab form={form} />}
      </div>
    </div>
  );
}
