...
-- ============================================
-- 11. FIX user_todos TABLE - ADD MISSING sort_order COLUMN
-- ============================================

-- Add the missing sort_order column to user_todos table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT FROM information_schema.columns
  ...
  ELSE
    RAISE NOTICE 'Column sort_order already exists in user_todos table';
  END IF;
END $$;
...
-- Update any existing todos to have a sort_order based on creation time
...
-- ============================================
-- 12. VERIFICATION FOR user_todos TABLE
-- ============================================

-- Verify user_todos sort_order column exists
SELECT
  'user_todos.sort_order column' AS check_name,
  CASE
    WHEN EXISTS (
      SELECT FROM information_schema.columns
      WHERE table_schema = 'public'
...
UNION ALL
SELECT
  'idx_user_todos_sort_order index' AS check_name,
  CASE
    WHEN EXISTS (
      SELECT FROM pg_indexes
      WHERE schemaname = 'public'
      AND tablename = 'user_todos'
      AND indexname = 'idx_user_todos_sort_order'
    ) THEN '✅ EXISTS'
    ELSE '❌ MISSING'
  END AS status;

-- Show current user_todos table structure
...
BEGIN
  ...
  RAISE NOTICE '   5. Verify sort_order column is working in todos';
END $$;
