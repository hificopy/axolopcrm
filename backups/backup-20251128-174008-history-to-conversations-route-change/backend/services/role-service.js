...
export async function searchRoleTemplates(query, category = null) {
  try {
    ...
    const { data, error } = await queryBuilder.order('category').order('position');
 ...
 */
export async function copyRoleTemplateToAgency(templateId, agencyId, createdBy) {
  try {
    // Get template
    ...
    if (existing) {
      return { success: false, error: 'A role with this name already exists in your agency' };
    }
...
export async function getMemberRoles(memberId) {
  try {
    ...
    if (error) throw error;
    return { success: true, data: data.map(mr => ({ ...mr.role, assignment_id: mr.id, assigned_at: mr.assigned_at })) };
  } catch (error) {
...
export async function getRoleMembers(roleId) {
  try {
    ...
    if (error) throw error;
    return { success: true, data: data.map(mr => ({ ...mr.member, assignment_id: mr.id, assigned_at: mr.assigned_at })) };
  } catch (error) {
...
export async function assignRoleToMember(memberId, roleId, assignedBy) {
  try {
    ...
    if (!member || !role || member.agency_id !== role.agency_id) {
      return { success: false, error: 'Member and role must be in the same agency' };
    }
 ...
 */
export async function setPermissionOverride(memberId, permissionKey, value, setBy, reason = null) {
  try {
    const { data, error } = await supabase
      .from('member_permission_overrides')
      .upsert({
 ...
 */
export async function setMultiplePermissionOverrides(memberId, overrides, setBy) {
  try {
