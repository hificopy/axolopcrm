...
import { X, Link2, Building2, Loader2, CheckCircle, AlertCircle, UserPlus } from 'lucide-react';
...
export default function JoinAgencyModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/agencies/invites/${agencyPreview.invite_code}/join`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json',
          ...
          <div className="flex items-center justify-between p-6 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Enter an invite link to join</p>
              </div>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1.5">
                Paste the invite link shared with you or just the 8-character code
              </p>
            </div>
            ...
            {agencyPreview && (
              <div className="p-4 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-green-400 font-medium">Valid Invite</span>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      You'll join as: <span className="text-purple-400 font-medium capitalize">{agencyPreview.role}</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-3 bg-gray-800/30 border border-white/5 rounded-lg">
              <p className="text-xs text-gray-400">
                <span className="text-gray-300 font-medium">How to get an invite:</span> Ask the agency admin to generate an invite link from their Agency Settings and share it with you.
              </p>
            </div>
          </div>
