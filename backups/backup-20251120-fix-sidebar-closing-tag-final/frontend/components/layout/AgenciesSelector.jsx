...
const AgenciesSelector = () => {
  ...
  return (
    <div className="px-3 py-2" style={{ marginLeft: '-8px' }}>
      <div className="text-xs text-gray-400 uppercase tracking-wide mb-1 px-3" style={{ marginLeft: '-10px' }}>Agencies</div>
        ...
        <div className="flex items-center">
          <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
              ...
              >
                <div className="flex items-center flex-1">
                  <div className={`p-2 rounded-lg mr-1 transition-colors duration-200 ease-out ${
                    'text-gray-400 hover:text-white'
                  }`}>
                    <Building2 className="h-4 w-4 transition-transform duration-200 ease-out" />
                  </div>
                  <span className={`text-sm font-bold transition-colors duration-200 ease-out text-gray-300 hover:text-white truncate`}>
                    {selectedAgency?.name}
                  </span>
                </div>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-56 bg-gradient-to-r from-gray-800/60 to-gray-900/60 backdrop-blur-lg border border-white/10 min-w-[240px] shadow-xl rounded-lg overflow-hidden">
