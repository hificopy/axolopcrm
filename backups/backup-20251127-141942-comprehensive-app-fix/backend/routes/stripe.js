...
import { requireAgencyAccess, requireAgencyOwner } from '../middleware/agency-access.js';
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    const { success, event, error } = await stripeService.constructWebhookEvent(
      req.body,
      signature
    );
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        result = await stripeService.handleCheckoutCompleted(event.data.object);
        break;

      case 'customer.subscription.updated':
        result = await stripeService.handleSubscriptionUpdated(event.data.object);
        break;

      case 'customer.subscription.deleted':
        result = await stripeService.handleSubscriptionDeleted(event.data.object);
        break;
      ...
      case 'invoice.payment_failed':
        result = await stripeService.handleInvoicePaymentFailed(event.data.object);
        break;
...
router.post('/create-portal-session',
  ...
  async (req, res) => {
    try {
      ...
      const result = await stripeService.createPortalSession(agencyId, returnUrl);
...
router.post('/cancel',
  ...
  async (req, res) => {
    try {
      ...
      const result = await stripeService.cancelSubscription(agencyId, !immediate);
...
router.post('/upgrade',
  ...
  async (req, res) => {
    try {
      ...
      const result = await stripeService.changeTier(agencyId, tier, billing_cycle);
