...
export default function Meetings() {
  ...
  const loadBookingLinks = useCallback(async () => {
    try {
      // Use the meetingsApi with automatic auth headers
  ...
  const loadGeneralAnalytics = useCallback(async () => {
    try {
      ...
      setGeneralAnalytics(response.data || {
        bookingTrends: [],
        cancellationRate: 0,
        rescheduleRate: 0,
        noShowRate: 0,
  ...
  const handleCreateLink = async (bookingData) => {
    try {
      // Use the meetingsApi with automatic auth headers
      const response = await meetingsApi.createBookingLink(bookingData);
  ...
  const handleSaveEditedLink = async (updatedData) => {
    try {
      // Use the meetingsApi with automatic auth headers
      await meetingsApi.updateBookingLink(editingLink.id, updatedData);
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to update booking link. Please try again.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // Use the meetingsApi with automatic auth headers
      await meetingsApi.deleteBookingLink(deletingLink.id);
      ...
      toast({
        title: 'Booking link deleted',
        description: 'The booking link has been permanently removed.',
      });
    } catch (error) {
      console.error('Error deleting booking link:', error);
      toast({
        title: 'Error',
        description: error.message || 'Failed to delete booking link. Please try again.',
        variant: 'destructive',
      });
    }
  };
                ...
                bookingLinks.map((link) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleDeleteClick(link)}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              All bookings associated with this link will remain in your calendar, but the booking page will no longer be accessible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeletingLink(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction
